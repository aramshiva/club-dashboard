...
def sanitize_string(value, max_length=None, allow_html=False):
    # if the text includes "');" then remove it
...
def sanitize_css_value(value, max_length=None):
    ...
    value = re.sub(
        r'url\s*\(\s*["\']?(?!https?:)[^)]*["\']?\s*\)', "", value, flags=re.IGNORECASE
    )
...
def suspend_user_for_security_violation(user, violation_type, details=""):
    ...
    try:
        ...
        app.logger.warning(
            f"SECURITY SUSPENSION - User {user.username} (ID: {user.id}) suspended for {violation_type}: {details}"
        )
        return True
...
def detect_exploit_attempts(text, field_context=""):
    ...
    is_content_field = any(
        keyword in field_context.lower()
        for keyword in ["assignment", "meeting", "resource", "club_post"]
    )
...
def detect_enumeration_attempts(text, field_context=""):
    ...
    if is_content_field:
        ...
        suspicious_files = [r"passwords?\.txt", r"secrets\.txt", r"\.env"]

        # Much higher threshold for content fields
        enum_count = sum(
            1 for pattern in enum_patterns if re.search(pattern, text_lower)
        )
        file_count = sum(
            1 for pattern in suspicious_files if re.search(pattern, text_lower)
        )
    ...
    try:
        ...
        if check_profanity_comprehensive(sanitized):
            ...
            if user and not user.is_admin:
                app.logger.warning(
                    f"PROFANITY DETECTED - User {user.username} (ID: {user.id}) used inappropriate language in {field_name}: {text[:100]}..."
                )
                create_audit_log(
            ...
            return (
                False,
                "Please remove inappropriate language from your content and try again.",
            )
...
HACKCLUB_IDENTITY_URL = os.getenv(
    "HACKCLUB_IDENTITY_URL", "https://identity.hackclub.com"
)
...
class AuditLog(db.Model):
    ...
    )  # signup, login, create_post, suspend_user, etc.
    action_category = db.Column(
        db.String(30), nullable=False, index=True
    )  # auth, user, club, admin, security
...
class OAuthApplication(db.Model):
    ...
    user = db.relationship(
        "User", backref=db.backref("oauth_applications", cascade="all, delete-orphan")
    )
...
class OAuthToken(db.Model):
    ...
    application_id = db.Column(
        db.Integer, db.ForeignKey("o_auth_application.id"), nullable=False
    )
    ...
    user = db.relationship(
        "User", backref=db.backref("oauth_tokens", cascade="all, delete-orphan")
    )
    application = db.relationship(
        "OAuthApplication", back_populates="tokens", foreign_keys=[application_id]
    )
...
class OAuthAuthorizationCode(db.Model):
    ...
    user = db.relationship(
        "User",
        backref=db.backref("oauth_authorization_codes", cascade="all, delete-orphan"),
    )
    application = db.relationship(
        "OAuthApplication",
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
                ...
                return jsonify(
                    {
                        ...
                        "received": f"Authorization: {auth_header[:50]}..."
            ...
            if not key_obj:
                ...
                inactive_key = APIKey.query.filter_by(
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not token_obj:
                ...
                inactive_token = OAuthToken.query.filter_by(
                    access_token=access_token, is_active=False
                ).first()
                if inactive_token:
                    return jsonify(
                        {
...
class Club(db.Model):
    ...
    __table_args__ = (
        ...
        db.CheckConstraint(
            "piggy_bank_tokens >= 0", name="check_piggy_bank_tokens_non_negative"
        ),
    )
    is_suspended = db.Column(db.Boolean, default=False, nullable=False)
    background_image_url = db.Column(
    ...
    co_leader = db.relationship(
        "User", foreign_keys=[co_leader_id], backref="co_led_clubs"
    )
...
class ClubCosmetic(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    club_id = db.Column(db.Integer, db.ForeignKey("club.id"), nullable=False)
    cosmetic_id = db.Column(
        db.String(100), nullable=False
    )  # e.g., 'rainbow_name', 'vip_role'
    cosmetic_type = db.Column(
        db.String(50), nullable=False
    )  # 'name_effect', 'role', 'badge', 'effect'
...
class MemberCosmetic(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    club_id = db.Column(db.Integer, db.ForeignKey("club.id"), nullable=False)
    club_cosmetic_id = db.Column(
        db.Integer, db.ForeignKey("club_cosmetic.id"), nullable=False
    )
    assigned_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    assigned_by = db.Column(
        db.Integer, db.ForeignKey("user.id"), nullable=False
    )  # Leader who assigned it
    is_active = db.Column(db.Boolean, default=True, nullable=False)
    ...
    user = db.relationship(
        ...
        backref=db.backref("member_cosmetics", lazy=True),
    )
    club = db.relationship("Club", backref=db.backref("member_cosmetics", lazy=True))
    club_cosmetic = db.relationship(
        "ClubCosmetic", backref=db.backref("member_assignments", lazy=True)
    )
...
class ClubTransaction(db.Model):
    ...
    transaction_type = db.Column(
        db.String(50), nullable=False
    )  # 'credit', 'debit', 'grant', 'purchase', 'refund', 'manual'
    amount = db.Column(
        db.Integer, nullable=False
    )  # Amount in tokens (positive for credits, negative for debits)
    description = db.Column(db.Text, nullable=False)
    balance_after = db.Column(
        db.Integer, nullable=False
    )  # Club balance after this transaction
    reference_id = db.Column(
        db.String(100), nullable=True
    )  # Reference to related record (project_id, order_id, etc.)
    reference_type = db.Column(
        db.String(50), nullable=True
    )  # 'project', 'shop_order', 'admin_action', etc.
    ...
    club = db.relationship(
        "Club",
        backref=db.backref(
            "transactions", lazy=True, order_by="ClubTransaction.created_at.desc()"
        ),
    )
    user = db.relationship(
        "User",
        foreign_keys=[user_id],
        backref=db.backref("club_transactions", lazy=True),
    )
...
class ClubSlackSettings(db.Model):
    ...
    club = db.relationship(
        "Club",
        backref=db.backref(
            "slack_settings", uselist=False, cascade="all, delete-orphan"
        ),
    )
...
class ClubChatMessage(db.Model):
    ...
    )  # 1000 char limit, nullable for image-only messages
    ...
    club = db.relationship(
        "Club",
        backref=db.backref(
            "chat_messages", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
    user = db.relationship(
        "User", backref=db.backref("club_chat_messages", lazy="dynamic")
    )
...
class AttendanceSession(db.Model):
    ...
    session_type = db.Column(
        db.String(50), default="meeting"
    )  # meeting, workshop, event, etc.
    ...
    club = db.relationship(
        "Club",
        backref=db.backref(
            "attendance_sessions", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
    ...
    def get_attendance_count(self):
        return AttendanceRecord.query.filter_by(
            session_id=self.id, status="present"
        ).count()
...
class AttendanceRecord(db.Model):
    ...
    marker = db.relationship(
        "User", foreign_keys=[marked_by], backref="marked_attendances"
    )

    # Unique constraint to prevent duplicate attendance records
    __table_args__ = (
        db.UniqueConstraint(
            "session_id", "user_id", name="unique_session_user_attendance"
        ),
    )
...
class AttendanceGuest(db.Model):
    ...
    session = db.relationship(
        "AttendanceSession", backref=db.backref("guests", cascade="all, delete-orphan")
    )
...
def create_club_transaction(
    ...
    try:
        ...
        if amount < 0 and club.tokens + amount < 0:
            return (
                False,
                f"Insufficient balance. Current: {club.tokens} tokens, Required: {abs(amount)} tokens",
            )
...
def update_quest_progress(club_id, quest_type, increment=1):
    ...
    try:
        ...
        quest = WeeklyQuest.query.filter_by(
            quest_type=quest_type, is_active=True
        ).first()
        if not quest:
        ...
        progress_record = ClubQuestProgress.query.filter_by(
            club_id=club_id, quest_id=quest.id, week_start=week_start
        ).first()
        ...
        if progress_record.progress >= target and not progress_record.completed:
            ...
            if club.piggy_bank_tokens >= quest.reward_tokens:
                ...
                if success:
                    ...
                    try:
                        ...
                        if piggy_success:
                            ...
                            app.logger.info(
                                f"Club {club_id} completed quest {quest.name} and received {quest.reward_tokens} tokens from piggy bank (remaining piggy bank: {club.piggy_bank_tokens})"
                            )
                        else:
                            app.logger.error(
                        ...
                        app.logger.error(
                            f"Error recording piggy bank transaction: {str(piggy_error)}"
                        )
                        # Still mark as claimed since the main transaction succeeded
                        progress_record.reward_claimed = True
                else:
                ...
                app.logger.warning(
                    f"Club {club_id} completed quest {quest.name} but piggy bank has insufficient tokens ({club.piggy_bank_tokens} < {quest.reward_tokens}). No reward given."
                )
...
class LeaderboardExclusion(db.Model):
    ...
    leaderboard_type = db.Column(
        db.String(50), nullable=False
    )  # 'total_tokens', 'monthly_tokens', etc.
    ...
    # Relationships
    club = db.relationship(
        "Club", backref=db.backref("leaderboard_exclusions", lazy=True)
    )
    excluded_by_user = db.relationship(
        "User", backref=db.backref("leaderboard_exclusions", lazy=True)
    )
...
class ProjectSubmission(db.Model):
    ...
    project_id = db.Column(
        db.Integer, db.ForeignKey("club_project.id"), nullable=True
    )  # Link to actual project if available
    ...
    user = db.relationship(
        "User",
        foreign_keys=[user_id],
        backref=db.backref("project_submissions", lazy=True),
    )
    club = db.relationship("Club", backref=db.backref("project_submissions", lazy=True))
    project = db.relationship(
        "ClubProject", backref=db.backref("submissions", lazy=True)
    )
    approver = db.relationship(
        "User",
        foreign_keys=[approved_by],
        backref=db.backref("approved_submissions", lazy=True),
    )
...
class ClubQuestProgress(db.Model):
    ...
    quest = db.relationship(
        "WeeklyQuest", backref=db.backref("progress_records", lazy=True)
    )

    __table_args__ = (
        db.UniqueConstraint(
            "club_id", "quest_id", "week_start", name="_club_quest_week_uc"
        ),
    )
...
class BlogPost(db.Model):
    ...
    category_id = db.Column(
        db.Integer, db.ForeignKey("blog_category.id"), nullable=True
    )
...
class SystemSettings(db.Model):
    ...
    updated_by_user = db.relationship(
        "User", backref=db.backref("system_settings_updates", lazy=True)
    )
        ...
        try:
            ...
            if setting:
                ...
                setting = SystemSettings(
                    key=key, value=str(value), updated_by=valid_user_id
                )
...
class StatusIncident(db.Model):
    ...
    description = db.Column(db.Text, nullable=False)
    status = db.Column(
        db.String(50), nullable=False, default="investigating"
    )  # investigating, identified, monitoring, resolved
    impact = db.Column(
        db.String(50), nullable=False, default="minor"
    ...
    creator = db.relationship(
        ...
        backref=db.backref("created_incidents", lazy=True),
    )
...
class StatusUpdate(db.Model):
    ...
    incident_id = db.Column(
        db.Integer, db.ForeignKey("status_incident.id"), nullable=False
    )
    ...
    incident = db.relationship(
        "StatusIncident",
        backref=db.backref("updates", lazy=True, order_by="StatusUpdate.created_at"),
    )
    creator = db.relationship(
        "User",
        foreign_keys=[created_by],
        backref=db.backref("status_updates", lazy=True),
    )
...
def is_user_co_leader(club, user):
    ...
    membership = ClubMembership.query.filter_by(
        club_id=club.id, user_id=user.id, role="co-leader"
    ).first()
...
def login_user(user, remember=False):
    ...
    try:
        db.session.commit()
        app.logger.info(
            f"User login: {user.username} (ID: {user.id}) from IP: {real_ip}"
        )
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        app.logger.debug(
            f"Auth check for {request.endpoint}: authenticated={authenticated}, user_id={session.get('user_id')}, logged_in={session.get('logged_in')}, current_user={current_user.username if current_user else None}"
        )

        if not authenticated or not current_user:
            app.logger.warning(
                f"Authentication failed for {request.endpoint}: user_id={session.get('user_id')}, logged_in={session.get('logged_in')}"
            )
            if request.is_json:
        ...
        if current_user.is_suspended and request.endpoint not in [
            "suspended",
            "logout",
        ]:
            if request.is_json:
...
class AirtableService:
    def __init__(self):
        ...
        self.clubs_base_id = os.environ.get(
            "AIRTABLE_CLUBS_BASE_ID", "appSUAc40CDu6bDAp"
        )
        self.clubs_table_id = os.environ.get(
            "AIRTABLE_CLUBS_TABLE_ID", "tbl5saCV1f7ZWjsn0"
        )
        self.clubs_table_name = os.environ.get(
            "AIRTABLE_CLUBS_TABLE_NAME", "Clubs Dashboard"
        )
        ...
        self.base_url = (
            f"https://api.airtable.com/v0/{self.base_id}/{encoded_table_name}"
        )

        # Club management URLs - use table ID for direct access
        self.clubs_base_url = (
            f"https://api.airtable.com/v0/{self.clubs_base_id}/{self.clubs_table_id}"
        )
    ...
    def _validate_airtable_url(self, url):
        ...
        try:
            ...
            return (
                ...
                and parsed.hostname == "api.airtable.com"
    ...
    def _check_school_variations(self, club_name, venue):
        ...
        venue_words = [
            word for word in venue.split() if word not in common_words and len(word) > 2
        ]
        ...
        for club_word in club_words:
            for venue_word in venue_words:
                if (
                    club_word in venue_word
                    or venue_word in club_word
                    or
                    # Check for common abbreviations
                    (club_word.startswith(venue_word[:3]) and len(venue_word) > 3)
                    or (venue_word.startswith(club_word[:3]) and len(club_word) > 3)
                ):
                    return True
    ...
    def verify_club_leader(self, email, club_name):
        ...
        try:
            ...
            app.logger.debug(
                f"Email filter formula: {email_filter_params['filterByFormula']}"
            )
            ...
            if parsed_url.hostname not in ["api.airtable.com"]:
                app.logger.error(
                    f"Invalid Airtable URL hostname: {parsed_url.hostname}"
                )
                return False
            ...
            app.logger.debug(
                f"Airtable response content length: {len(response.content) if response.content else 0}"
            )
            ...
            if response.status_code == 200:
                try:
                    data = response.json()
                    app.logger.debug(
                        f"Airtable response data keys: {list(data.keys()) if data else 'None'}"
                    )
                    records = data.get("records", [])
                    app.logger.info(f"Found {len(records)} records with email {email}")
                    if records:
                        app.logger.debug(
                            f"First record fields: {list(records[0].get('fields', {}).keys()) if records else 'None'}"
                        )
                except ValueError as json_error:
                    app.logger.error(
                        f"Failed to parse Airtable JSON response: {json_error}"
                    )
                ...
                club_names = [
                    record.get("fields", {}).get("Club Name", "") for record in records
                ]
                app.logger.info(f"Available club names for {email}: {club_names}")
                app.logger.debug(
                    f"Full record data for debugging: {[record.get('fields', {}) for record in records]}"
                )
                ...
                for record in records:
                    ...
                    app.logger.debug(
                        f"Checking club name: '{venue}' against requested club name: '{club_name_lower}'"
                    )

                    # Try multiple matching strategies with more flexible matching
                    if (
                        club_name_lower in venue
                        or venue.find(club_name_lower) >= 0
                        or
                        # Check if club name words are in venue
                        any(
                            word.strip() in venue
                            for word in club_name_lower.split()
                            if len(word.strip()) > 2
                        )
                        or
                        # Check if venue words are in club name
                        any(
                            word.strip() in club_name_lower
                        ...
                        app.logger.info(
                            f"Found matching club: {fields.get('Club Name', '')}"
                        )
                        return True

                app.logger.info(
                    f"No club name match found for '{club_name}' in available clubs: {club_names}"
                )
                return False

            elif response.status_code == 403:
                app.logger.error(
                    f"Airtable 403 Forbidden - check API token permissions. Response: {response.text}"
                )
                return False
            elif response.status_code == 404:
                app.logger.error(
                    f"Airtable 404 Not Found - check base ID and table name. Response: {response.text}"
                )
                return False
            else:
                app.logger.error(
        ...
        try:
            ...
            if grant_amount > 0:
                ...
                if not is_in_person:
                    ...
                    grant_amount = 0
                    app.logger.info(
                        f"Grant denied: Club has {club_member_count} members, need 3+"
                    )
                else:
                    app.logger.info(
                        f"Grant approved: ${grant_amount} for {hours} hours (in-person meeting, {club_member_count} members)"
                    )
            ...
            project_url = (
                f"https://api.airtable.com/v0/{self.base_id}/{project_table_name}"
            )
            ...
            fields = {
                ...
                "Screenshot": [{"url": submission_data.get("screenshot_url", "")}]
                if submission_data.get("screenshot_url")
                else [],
                ...
                "In-Person Meeting": "Yes"
                if submission_data.get("is_in_person_meeting", False)
                else "No",
                "Club Member Count": str(submission_data.get("club_member_count", 0)),
                "Meeting Requirements Met": "Yes"
                if (
                    submission_data.get("is_in_person_meeting", False)
                    and submission_data.get("club_member_count", 0) >= 3
                )
                else "No",
            }
            ...
            app.logger.debug(
                f"Club name in submission_data: '{submission_data.get('club_name', 'NOT_FOUND')}'"
            )
            app.logger.debug(
                f"Leader email in submission_data: '{submission_data.get('leader_email', 'NOT_FOUND')}'"
            )
            ...
            if filtered_out:
                app.logger.debug(
                    f"Fields filtered out due to empty values: {filtered_out}"
                )
            ...
            app.logger.info(
                f"Screenshot field value: {fields.get('Screenshot', 'NOT_FOUND')}"
            )
    ...
    def submit_purchase_request(self, purchase_data):
        ...
        fulfillment_url = (
            f"https://api.airtable.com/v0/{self.base_id}/{fulfillment_table_name}"
        )
        ...
        try:
            response = requests.post(
                fulfillment_url, headers=self.headers, json=payload
            )
            app.logger.debug(
                f"Airtable Grant Fulfillment response status: {response.status_code}"
            )
            app.logger.debug(
                f"Airtable Grant Fulfillment response body: {response.text}"
            )
            if response.status_code in [200, 201]:
                return response.json()
            else:
            ...
            app.logger.error(
                f"Exception submitting to Airtable Grant Fulfillment: {str(e)}"
            )
            return None
                    ...
                    submissions.append(
                        {
                            ...
                            "club_name": fields.get(
                                "Club Name", fields.get("Hack Club", "")
                            ),
                            ...
                            "grant_amount": fields.get("Grant Amount Override")
                            or fields.get("Grant Amount", ""),
                            "status": fields.get(
                                "Status",
                                fields.get(
                                    "Grant Status",
                                    fields.get("Review Status", "Pending"),
                                ),
                            ),
    ...
    def update_submission_status(self, submission_id, action):
        ...
        try:
            ...
            for field_name in possible_status_fields:
                update_data = {"fields": {field_name: status}}

                response = requests.patch(url, headers=self.headers, json=update_data)

                if response.status_code == 200:
                    app.logger.info(
                        f"Submission {submission_id} status updated to {status} using field '{field_name}'"
                    )
                    return True
                else:
                    app.logger.debug(
                        f"Failed to update with field '{field_name}': {response.status_code} - {response.text}"
                    )

            # If no field worked, log the error and return False
            app.logger.error(
                f"Failed to update submission status with any field name. Last response: {response.status_code} - {response.text}"
            )
            return False
        except Exception as e:
    ...
    def get_all_clubs_from_airtable(self):
        ...
        if not self.api_token:
            app.logger.error(
                "Cannot fetch clubs from Airtable: API token not configured"
            )
            return []
        ...
        try:
            ...
            while True:
                ...
                response = requests.get(
                    self.clubs_base_url, headers=self.headers, params=params
                )
                app.logger.debug(
                    f"Page {page_count} response status: {response.status_code}"
                )

                if response.status_code != 200:
                    app.logger.error(
                        f"Airtable API error on page {page_count}: {response.status_code} - {response.text}"
                    )
                ...
                try:
                    ...
                    app.logger.debug(
                        f"Page {page_count}: Retrieved {len(page_records)} records, total so far: {len(all_records)}"
                    )

                    offset = data.get("offset")
                    if not offset:
                        app.logger.info(
                            f"Completed fetching all clubs from Airtable. Total records: {len(all_records)}"
                        )
                        break
                except ValueError as json_error:
                    app.logger.error(
                        f"Failed to parse Airtable JSON response on page {page_count}: {json_error}"
                    )
                    app.logger.error(f"Raw response content: {response.text[:500]}...")
                    break
            ...
            clubs = []
            app.logger.debug(
                f"Processing {len(all_records)} Airtable records into club data"
            )
            for i, record in enumerate(all_records):
                fields = record.get("fields", {})
                app.logger.debug(
                    f"Processing record {i + 1}/{len(all_records)}: ID={record.get('id')}, Fields keys: {list(fields.keys())}"
                )
                ...
                club_data = {
                    ...
                    .split(",")[0]
                    .strip()
                    if fields.get("Current Leaders' Emails")
                    else "",
                    ...
                    "club_applications_link": fields.get(
                        "Club Applications Link", ""
                    ).strip(),
                }
                ...
                if club_data["name"] and club_data["leader_email"]:
                    ...
                    app.logger.debug(
                        f"Added valid club: {club_data['name']} ({club_data['leader_email']})"
                    )
                else:
                    app.logger.debug(
                        f"Skipped invalid club record - Name: '{club_data['name']}', Email: '{club_data['leader_email']}'"
                    )

            app.logger.info(
                f"Successfully processed {len(clubs)} valid clubs from {len(all_records)} Airtable records"
            )
            return clubs
    ...
    def sync_club_with_airtable(self, club_id, airtable_data):
        ...
        try:
            ...
            app.logger.debug(
                f"Airtable data keys: {list(airtable_data.keys()) if airtable_data else 'None'}"
            )
            ...
            app.logger.debug(
                f"Found club: {club.name} (current location: {club.location})"
            )
            ...
            if "description" in airtable_data and airtable_data["description"]:
                filtered_description = filter_profanity_comprehensive(
                    airtable_data["description"]
                )
            ...
            club.airtable_data = json.dumps(
                {
                    ...
                    "club_applications_link": airtable_data.get(
                        "club_applications_link"
                    ),
                }
            )
            ...
            db.session.commit()
            app.logger.info(
                f"Successfully synced club {club_id} ({club.name}) with Airtable data"
            )
            return True
    ...
    def create_club_from_airtable(self, airtable_data):
        ...
        try:
            ...
            if not leader_email:
                app.logger.error(
                    "Cannot create club: no leader email provided in Airtable data"
                )
                return None
            ...
            if not leader:
                ...
                leader = User(
                    username=username,
                    email=leader_email,
                    first_name=airtable_data.get("primary_leader", "").split(" ")[0]
                    if airtable_data.get("primary_leader")
                    else "",
                    last_name=" ".join(
                        airtable_data.get("primary_leader", "").split(" ")[1:]
                    )
                    if airtable_data.get("primary_leader")
                    else "",
                )
            ...
            if existing_club:
                app.logger.warning(
                    f"Skipping club creation from Airtable - duplicate name: {filtered_name}"
                )
                return None
            ...
            app.logger.info(
                f"Successfully created club '{club.name}' from Airtable data (ID: {club.id})"
            )
            return club
            ...
            try:
                ...
                if existing_response.status_code == 200:
                    ...
                    if existing_records:
                        ...
                        payload = {
                            "fields": {"Code": verification_code, "Status": "Pending"}
                        }
                ...
                if response.status_code in [200, 201]:
                    ...
                    app.logger.error(
                        f"Failed to send email verification: {response.status_code} - {response.text}"
                    )
                    return None
                ...
                retry_count += 1
                app.logger.warning(
                    f"Email verification timeout, attempt {retry_count}/{max_retries}: {str(e)}"
                )
                if retry_count >= max_retries:
                    app.logger.error(
                        f"Email verification failed after {max_retries} attempts due to timeout"
                    )
                    return None
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if records:
                    ...
                    payload = {"fields": {"Status": "Verified"}}

                    update_response = self._safe_request(
                        "PATCH",
                        update_url,
                        headers=self.headers,
                        json=payload,
                    ...
                    if update_response.status_code == 200:
                        ...
                        app.logger.error(
                            f"Failed to update verification status: {update_response.status_code}"
                        )
                        return False
                else:
                    app.logger.warning(
                        f"No pending verification found for {email} with code {code}"
                    )
                    return False
            else:
                app.logger.error(
                    f"Error checking verification code: {response.status_code} - {response.text}"
                )
                return False
    ...
    def submit_project_data(self, submission_data):
        ...
        try:
            ...
            app.logger.info(
                f"AIRTABLE: Project name: {fields.get('Hackatime Project', 'NOT_FOUND')}"
            )
            ...
            response = self._safe_request(
                "POST", project_url, headers=self.headers, json=payload
            )
            ...
            app.logger.info(
                f"AIRTABLE: Successfully submitted project! Record ID: {result.get('records', [{}])[0].get('id', 'UNKNOWN')}"
            )
            return result
    ...
    def get_ysws_project_submissions(self):
        ...
        try:
            ...
            while True:
                ...
                response = self._safe_request(
                    "GET", project_url, headers=self.headers, params=params
                )

                if response.status_code != 200:
                    app.logger.error(
                        f"AIRTABLE: Failed to fetch project submissions: {response.text}"
                    )
                    break
            ...
            for record in all_records:
                ...
                submission = {
                    ...
                    "grantAmount": fields.get("Grant Amount Override")
                    or fields.get("Grant Amount", ""),
                ...
                if (
                    isinstance(submission["screenshot"], list)
                    and len(submission["screenshot"]) > 0
                ):
                    submission["screenshot"] = submission["screenshot"][0].get(
                        "url", ""
                    )
            ...
            app.logger.error(
                f"AIRTABLE: Exception in get_ysws_project_submissions: {str(e)}"
            )
            return []
    ...
    def update_ysws_project_submission(self, record_id, fields):
        ...
        if not self.api_token or not record_id:
            app.logger.error(
                "AIRTABLE: API token not configured or no record ID provided"
            )
            return False
        ...
        try:
            ...
            allowed_fields = {
                "Status",
                "Decision Reason",
                "Grant Amount Override",
                "Auto Review Status",
                "Grant Override Reason",
            }
            ...
            response = self._safe_request(
                "PATCH", update_url, headers=self.headers, json=payload
            )

            if response.status_code == 200:
                app.logger.info(
                    f"AIRTABLE: Successfully updated project submission {record_id}"
                )
                return True
            else:
                app.logger.error(
                    f"AIRTABLE: Failed to update project submission: {response.text}"
                )
                return False

        except Exception as e:
            app.logger.error(
                f"AIRTABLE: Exception in update_ysws_project_submission: {str(e)}"
            )
            return False
    ...
    def delete_ysws_project_submission(self, record_id):
        ...
        try:
            ...
            if response.status_code == 200:
                app.logger.info(
                    f"AIRTABLE: Successfully deleted project submission {record_id}"
                )
                return True
            else:
                app.logger.error(
                    f"AIRTABLE: Failed to delete project submission: {response.text}"
                )
                return False

        except Exception as e:
            app.logger.error(
                f"AIRTABLE: Exception in delete_ysws_project_submission: {str(e)}"
            )
            return False
    ...
    def get_orders_for_club(self, club_name):
        ...
        try:
            # Filter by club name
            params = {"filterByFormula": f"{{Club Name}} = '{club_name}'"}
                    ...
                    orders.append(
                        {
                            ...
                            "total_estimated_cost": fields.get(
                                "Total Estimated Cost", 0
                            ),
                            "delivery_address_line_1": fields.get(
                                "Delivery Address Line 1", ""
                            ),
                            "delivery_address_line_2": fields.get(
        ...
        try:
            ...
            while True:
                ...
                if response.status_code == 200:
                        ...
                        all_orders.append(
                            {
                                "id": record["id"],
                                "club_name": fields.get("Club Name", ""),
                                "leader_first_name": fields.get(
                                    "Leader First Name", ""
                                ),
                                "leader_last_name": fields.get("Leader Last Name", ""),
                                "leader_email": fields.get("Leader Email", ""),
                                "club_member_amount": fields.get(
                                    "Club Member Amount", 0
                                ),
                                "products": fields.get("Product(s)", ""),
                                "total_estimated_cost": fields.get(
                                    "Total Estimated Cost", 0
                    ...
                    app.logger.error(
                        f"Failed to fetch all orders: {response.status_code} - {response.text}"
                    )
                    break
    ...
    def update_order_status(self, order_id, status, reviewer_reason):
        ...
        orders_table_name = urllib.parse.quote("Orders")
        update_url = (
            f"https://api.airtable.com/v0/{shop_base_id}/{orders_table_name}/{order_id}"
        )

        fields = {"Shipment Status": status, "Reviewer Reason": reviewer_reason}
        ...
        try:
            response = requests.patch(update_url, headers=self.headers, json=payload)
            app.logger.debug(
                f"Airtable order update response status: {response.status_code}"
            )
    ...
    def delete_order(self, order_id):
        ...
        try:
            ...
            app.logger.debug(
                f"Airtable order delete response status: {response.status_code}"
            )
    ...
    def log_gallery_post(
        self, post_title, description, photos, club_name, author_username
    ):
        ...
        try:
            ...
            gallery_table_name = urllib.parse.quote(
                "Gallary"
            )  # Table name provided by user (note the spelling)
            gallery_url = (
                f"https://api.airtable.com/v0/{gallery_base_id}/{gallery_table_name}"
            )
            ...
            response = self._safe_request(
                "POST", gallery_url, headers=self.headers, json=payload
            )

            app.logger.info(
                f"AIRTABLE: Gallery post response status: {response.status_code}"
            )

            if response.status_code == 200:
                result = response.json()
                app.logger.info(
                    f"AIRTABLE: Successfully logged gallery post! Record ID: {result.get('id', 'UNKNOWN')}"
                )
                return True
            else:
                app.logger.error(
                    f"AIRTABLE: Gallery post logging failed: {response.text}"
                )
                return False
...
class HackatimeService:
    ...
    def get_user_stats(self, api_key):
        ...
        app.logger.info(
            f"get_user_stats: Making request to Hackatime API with key {masked_key}"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                app.logger.info(
                    f"get_user_stats: Success - received data with keys: {list(data.keys()) if isinstance(data, dict) else 'non-dict response'}"
                )
                if isinstance(data, dict) and "data" in data:
                    projects_count = len(data["data"].get("projects", []))
                    app.logger.info(
                        f"get_user_stats: Found {projects_count} projects in response"
                    )
                else:
                    app.logger.warning(
                        "get_user_stats: Response missing 'data' key or not a dict"
                    )
                return data
            else:
                app.logger.error(
                    f"get_user_stats: API request failed with status {response.status_code}"
                )
                try:
                    error_body = response.text[:500]  # Limit error body length
                    app.logger.error(
                        f"get_user_stats: Error response body: {error_body}"
                    )
                except:
                    app.logger.error(
                        "get_user_stats: Could not read error response body"
                    )
                return None
    ...
    def get_user_projects(self, api_key):
        ...
        if "data" not in stats:
            app.logger.warning(
                f"get_user_projects: stats missing 'data' key. Stats keys: {list(stats.keys())}"
            )
            return []
        ...
        for i, project in enumerate(projects[:5]):  # Log first 5 projects
            ...
            app.logger.info(
                f"get_user_projects: Project {i + 1}: '{project_name}' with {total_seconds} seconds"
            )

        active_projects = [p for p in projects if p.get("total_seconds", 0) > 0]
        app.logger.info(
            f"get_user_projects: {len(active_projects)} projects have activity (>0 seconds)"
        )
        ...
        app.logger.info(
            f"get_user_projects: Returning {len(active_projects)} active projects"
        )
    ...
    def get_user_identity(self, access_token):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                app.logger.warning(
                    f"Identity API error: {response.status_code} - {response.text}"
                )
                return None
        except Exception as e:
    ...
    def get_user_info(self, access_token):
        ...
        try:
            ...
            profile_response = requests.get(
                profile_url, headers=headers, params=profile_params
            )
            if profile_response.status_code == 200:
                try:
                    profile_data = profile_response.json()
                    if profile_data.get("ok"):
                        identity_data["user"]["profile"] = profile_data["user"][
                            "profile"
                        ]
                except:
                    pass
...
def slack_callback():
    ...
    if user:
        app.logger.info(
            f"Slack OAuth: User {user.username} (ID: {user.id}) logging in from IP: {request.remote_addr}"
        )
        login_user(user, remember=True)
        app.logger.info(
            f"Slack OAuth: Session created for user {user.username}: session_id={session.get('user_id')}, logged_in={session.get('logged_in')}"
        )
        ...
        if pending_join_code:
            ...
            return redirect(
                url_for("join_club_redirect") + f"?code={pending_join_code}"
            )
...
def hackclub_identity_authorize():
    ...
    redirect_uri = url_for(
        "hackclub_identity_callback", _external=True, _scheme="https"
    )
    state = secrets.token_urlsafe(32)
...
def check_maintenance_mode():
    ...
    if request.endpoint and (
        request.endpoint.startswith("static") or "/api/" in request.path
    ):
        return
        ...
        try:
            if not SystemSettings.is_economy_enabled():
                ...
                if (
                    ...
                    and SystemSettings.is_admin_economy_override_enabled()
                ):
...
def inject_system_settings():
    ...
    try:
        ...
        current_user = get_current_user()
        effective_economy_enabled = economy_enabled or (
            admin_economy_override and current_user and current_user.is_admin
        )
        ...
        return dict(
            ...
            economy_actually_enabled=True,
            admin_economy_override=False,
...
def hackclub_identity_callback():
    ...
    identity_info = hackclub_identity_service.get_user_identity(
        current_user.identity_token
    )

    app.logger.info(f"Identity info received: {identity_info}")

    if identity_info and "identity" in identity_info:
        verification_status = identity_info["identity"].get(
        ...
        if verification_status == "verified":
            ...
            rejection_reason = identity_info["identity"].get(
...
def hackclub_identity_status():
    ...
    return jsonify(
        {"status": "error", "verified": False, "message": "Failed to check status"}
    )
            ...
            try:
                ...
                try:
                    ...
                    if summary_response.status_code == 200:
                        ...
                        app.logger.warning(
                            f"AI summary API returned {summary_response.status_code}"
                        )
                except Exception as summary_error:
                    app.logger.warning(
                        f"Failed to generate AI summary: {summary_error}"
                    )
                ...
                if console_logs:
                    try:
                        ...
                        for log in recent_logs:
                            timestamp = log.get("timestamp", "unknown")[:19].replace(
                                "T", " "
                        ...
                        app.logger.warning(
                            f"Failed to format console logs: {log_error}"
                        )
                        formatted_logs = "Error formatting console logs"

                # Submit to Airtable
                airtable_api_key = os.getenv("AIRTABLE_API_KEY") or os.getenv(
                    "AIRTABLE_TOKEN"
                )
                ...
                try:
                    airtable_url = (
                        "https://api.airtable.com/v0/app7OFpfZceddfK17/Support"
                    )
                    airtable_headers = {
                    ...
                    if formatted_logs:
                        airtable_data["fields"]["Logs"] = formatted_logs[
                            :10000
                        ]  # Limit to 10k chars for Airtable
                    ...
                    if airtable_response.status_code in [200, 201]:
                        app.logger.info(
                            f"Successfully submitted to Airtable: {airtable_response.json()}"
                        )
                        return True
                    else:
                        app.logger.error(
                            f"Airtable API error: {airtable_response.status_code} - {airtable_response.text}"
                        )
                        return False
...
"""
        ...
        messages = [
            {
                "role": "system",
                "content": heidi_config.get("system_prompt", "") + user_context,
            }
        ]
        ...
        if ai_response.status_code == 200:
            ...
            assistant_message = message_data.get(
                ...
                for tool_call in tool_calls:
                    if (
                        ...
                        try:
                            ...
                            issue_summary = args.get(
                                "issue_summary", "No summary provided"
                            )

                            app.logger.info(
                                f"Escalation triggered - Reason: {escalation_reason}, Summary: {issue_summary}"
                            )
                            ...
                            app.logger.error(
                                f"Error processing escalation tool call: {tool_error}"
                            )
            ...
            if (
                assistant_message
                and "<thinking>" in assistant_message
                and "</thinking>" in assistant_message
            ):
                ...
                assistant_message = re.sub(
                    r"<thinking>.*?</thinking>", "", assistant_message, flags=re.DOTALL
                ).strip()

            # Remove any other debug markers
            if (
                assistant_message
                and assistant_message.startswith("Okay,")
                and "I should respond" in assistant_message
            ):
                ...
                for line in lines:
                    if not capturing and any(
                        ...
                        for marker in ["i'm", "hello", "hi", "thanks", "great"]
                    ):
                        capturing = True
                    if capturing and not any(
                        debug in line.lower()
                        for debug in [
                            "should respond",
                            "keeping it",
            ...
            app.logger.error(
                f"AI API error: {ai_response.status_code} - {ai_response.text}"
            )
...
def get_heidi_config():
    ...
    try:
        ...
        frontend_config = {
            ...
            "greeting": config.get(
                "greeting",
                "Hi! I'm Heidi, your AI assistant. How can I help you today?",
            ),
...
def login():
    ...
    if request.method == "POST":
        email = (
            sanitize_string(request.form.get("email", ""), max_length=120)
            .strip()
            .lower()
        )
        ...
        if user and user.check_password(password):
            app.logger.info(
                f"User {user.username} (ID: {user.id}) logging in from IP: {request.remote_addr}"
            )
            login_user(user, remember=remember_me)
            app.logger.info(
                f"Session created for user {user.username}: session_id={session.get('user_id')}, logged_in={session.get('logged_in')}"
            )
            flash(f"Welcome back, {user.username}!", "success")
...
def signup():
    ...
    if request.method == "POST":
        ...
        username = sanitize_string(
            request.form.get("username", ""), max_length=30
        ).strip()
        email = sanitize_string(request.form.get("email", ""), max_length=120).strip()
        password = request.form.get("password", "")
        first_name = sanitize_string(
            request.form.get("first_name", ""), max_length=50
        ).strip()
        last_name = sanitize_string(
            request.form.get("last_name", ""), max_length=50
        ).strip()
...
def dashboard():
    ...
    all_club_ids = set(
        [club.id for club in led_clubs] + [m.club.id for m in memberships]
    )
    ...
    return render_template(
        "dashboard.html", memberships=memberships, led_clubs=led_clubs
    )
...
def club_dashboard(club_id=None):
    ...
    # Check if mobile dashboard is enabled
    if (
        (is_mobile or force_mobile)
        and not force_desktop
        and not SystemSettings.is_mobile_enabled()
    ):
    ...
    banner_settings = {
        ...
        "subtitle": SystemSettings.get_setting(
            "banner_subtitle", "Submit your creative projects and win amazing prizes!"
        ),
        ...
        "secondary_color": SystemSettings.get_setting(
            "banner_secondary_color", "#d63146"
        ),
        "background_color": SystemSettings.get_setting(
    ...
    if is_leader or is_co_leader or is_member:
        ...
        effective_is_co_leader = (
            is_co_leader or is_admin_access
        )  # Admin acts as co-leader minimum
        effective_can_manage = (
            is_leader or is_co_leader or is_admin_access
        )  # For general management tasks

        if (is_mobile or force_mobile) and not force_desktop:
            return render_template(
                "club_dashboard_mobile.html",
                club=club,
    ...
    if request.method == "POST":
        ...
        if step == "send_verification":
            ...
            if not airtable_service.api_token:
                app.logger.error(
                    "Airtable verification failed: API token not configured"
                )
                return jsonify(
                    {
                        "error": "Club verification service is not configured. Please contact support."
                    }
                ), 500
            ...
            try:
                ...
                app.logger.debug(
                    f"Using filter formula: {email_filter_params['filterByFormula']}"
                )

                response = requests.get(
                    airtable_service.clubs_base_url,
                ...
                if response.status_code == 200:
                    ...
                    return jsonify(
                        {"error": "Failed to fetch clubs from directory"}
                    ), 500
            except Exception as e:
            ...
            return jsonify({"success": True, "message": "Club linked successfully!"})
...
def get_shop_items(club_id):
    ...
    try:
        ...
        if disabled_response.status_code == 200:
            ...
            app.logger.info(
                f"Found {len(disabled_record_ids)} disabled items: {[r.get('fields', {}).get('Item', 'Unknown') for r in disabled_records]}"
            )
        else:
            app.logger.warning(
                f"Failed to fetch disabled items filter: {disabled_response.status_code}"
            )

        app.logger.info(
            f"Fetched {len(all_records)} total shop items from Airtable, {len(disabled_record_ids)} are disabled"
        )
        ...
        for record in all_records:
            ...
            app.logger.info(
                f"🛍️ Shop item '{fields.get('Item', 'Unknown')}' (ID: {record_id}) - Disabled: {is_disabled}"
            )
            ...
            if item["name"] and item["price"] and item["enabled"]:
                items.append(item)
                app.logger.info(
                    f"✅ Including shop item: {item['name']} (enabled: {item['enabled']})"
                )
            else:
                app.logger.info(
                    f"❌ Excluding shop item: {item['name']} (name: {bool(item['name'])}, price: {bool(item['price'])}, enabled: {item['enabled']})"
                )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for record in records:
                ...
                if "Picture" in fields and fields["Picture"]:
                    if (
                        isinstance(fields["Picture"], list)
                        and len(fields["Picture"]) > 0
                    ):
                        picture_url = fields["Picture"][0].get("url", "")
                    elif isinstance(fields["Picture"], str):
                        picture_url = fields["Picture"]
...
def submit_order(club_id):
    ...
    if not is_leader and not is_co_leader:
        return jsonify(
            {"error": "Only club leaders and co-leaders can place orders"}
        ), 403
    ...
    if result:
        ...
        if success:
            app.logger.info(
                f"Transaction recorded for shop order: {total_tokens} tokens deducted"
            )
            ...
            app.logger.error(
                f"Failed to record transaction for shop order: {tx_result}"
            )
...
def purchase_cosmetic(club_id):
    ...
    if not is_leader and not is_co_leader:
        return jsonify(
            {"error": "Only club leaders and co-leaders can purchase cosmetics"}
        ), 403
    ...
    try:
        ...
        if existing_cosmetic:
            return jsonify(
                {"error": "This cosmetic has already been purchased for this club"}
            ), 400
        ...
        cosmetic = ClubCosmetic(
            club_id=club_id,
            cosmetic_id=sanitize_string(
                data.get(
                    "cosmetic_id",
                    data.get("cosmetic_name", "").lower().replace(" ", "_"),
                ),
                100,
            ),
        ...
        app.logger.info(
            f"Cosmetic purchased: {data.get('cosmetic_name')} for club {club_id} by user {current_user.id}"
        )
...
def get_club_slack_settings(club_id):
    ...
    if not is_leader and not is_co_leader:
        return jsonify(
            {"error": "Only club leaders and co-leaders can access Slack settings"}
        ), 403
...
def update_club_slack_settings(club_id):
    ...
    if not is_leader and not is_co_leader:
        return jsonify(
            {"error": "Only club leaders and co-leaders can update Slack settings"}
        ), 403
    ...
    if channel_id and not channel_id.startswith("C"):
        return jsonify(
            {
                "error": 'Invalid channel ID format. Slack channel IDs should start with "C"'
            }
        ), 400
...
def invite_member_to_slack(club_id):
    ...
    if not is_leader and not is_co_leader:
        return jsonify(
            {"error": "Only club leaders and co-leaders can invite members to Slack"}
        ), 403
    ...
    if not slack_settings or not slack_settings.channel_id:
        return jsonify(
            {
                "error": "Club Slack channel not configured. Please set up the channel first."
            }
        ), 400
    ...
    try:
        ...
        payload = {"email": email, "channel_id": slack_settings.channel_id}

        response = requests.post(slack_api_url, json=payload, timeout=30)

        if response.status_code == 200:
            app.logger.info(
                f"Successfully invited {email} to Slack channel {slack_settings.channel_id} for club {club_id}"
            )
            return jsonify(
                {
            ...
            app.logger.error(
                f"Slack invitation failed: {response.status_code} - {response.text}"
            )
            return jsonify(
                {
        ...
        return jsonify(
            {"error": "Failed to connect to Slack service. Please try again later."}
        ), 500
    except Exception as e:
...
def bulk_invite_members_to_slack(club_id):
    ...
    try:
        ...
        for email in member_emails:
            try:
                ...
                if response.status_code == 200:
                    success_count += 1
                    app.logger.info(
                        f"Successfully invited {email} to Slack channel {slack_settings.channel_id}"
                    )
                else:
                    failed_invitations.append(email)
                    app.logger.warning(
                        f"Failed to invite {email} to Slack: {response.status_code} - {response.text}"
                    )
        ...
        app.logger.error(
            f"Error during bulk Slack invitation for club {club_id}: {str(e)}"
        )
        return jsonify(
            {
                "error": "An unexpected error occurred during bulk invitation. Please try again."
            }
        ), 500
...
def project_submission(club_id):
    ...
    has_hackatime = bool(
        current_user.hackatime_api_key and current_user.hackatime_api_key.strip()
    )
...
def leaderboard(leaderboard_type="total"):
    ...
    try:
        ...
        exclusions = LeaderboardExclusion.query.filter_by(
            leaderboard_type=exclusion_type
        ).all()
        ...
        club_totals = (
            ...
            .group_by(ClubTransaction.club_id)
            .subquery()
        )
        ...
        if leaderboard_type == "per_member":
            ...
            project_counts = (
                ...
                .group_by(ProjectSubmission.club_id)
                .subquery()
            )
            ...
            leaderboard_data = (
                ...
                .outerjoin(project_counts, Club.id == project_counts.c.club_id)
                ...
                .group_by(Club.id, Club.name, project_counts.c.total_projects)
                .having(
            ...
            leaderboard_data = (
                ...
                .outerjoin(club_totals, Club.id == club_totals.c.club_id)
                .outerjoin(ClubMembership, Club.id == ClubMembership.club_id)
                .filter(
                    ~Club.id.in_(excluded_club_ids),
                    Club.is_suspended == False,  # Exclude suspended clubs
                )
                .group_by(Club.id, Club.name, club_totals.c.total_tokens)
                .order_by(club_totals.c.total_tokens.desc().nullslast())
                .limit(50)
                .all()
            )
            ...
            # Format the data
            clubs = []
            for rank, (club_id, name, total_tokens, member_count) in enumerate(
                leaderboard_data, 1
            ):
                clubs.append(
                    {
...
def complete_leader_signup():
    ...
    if (
        not leader_verification
        or not leader_verification.get("club_verified")
        or not leader_verification.get("email_verified")
    ):
        flash(
            "Invalid verification session. Please complete both club and email verification.",
            "error",
        )
        return redirect(url_for("verify_leader"))
    ...
    try:
                    ...
                    for record in records:
                        ...
                        if (
                            ...
                            club_data = {
                                ...
                                "location": fields.get("Location", "").strip(),
                                "description": fields.get("Description", "").strip()
                                or f"Official {fields.get('Club Name', '')} Hack Club",
                                "status": fields.get("Status", "").strip(),
                                "meeting_day": fields.get("Meeting Day", "").strip(),
                                "meeting_time": fields.get("Meeting Time", "").strip(),
                                "website": fields.get("Website", "").strip(),
                                "slack_channel": fields.get(
                                    "Slack Channel", ""
                                ).strip(),
                                "github": fields.get("GitHub", "").strip(),
                                "latitude": fields.get("Latitude"),
                                ...
                                "timezone": fields.get("Timezone", "").strip(),
                                "primary_leader": fields.get(
                                    "Current Leader(s)", ""
                                ).strip(),
                                "co_leaders": fields.get("Co-Leaders", "").strip(),
                                "meeting_notes": fields.get(
                                    "Meeting Notes", ""
                                ).strip(),
                                "club_applications_link": fields.get(
                                    "Application Link", ""
        ...
        if club_data:
            existing_airtable_club = Club.query.filter(
                Club.airtable_data.contains(
                    f'"airtable_id": "{club_data["airtable_id"]}"'
                )
            ).first()
        ...
        if existing_club:
            ...
            if club_data:
                ...
                try:
                    ...
                    airtable_update_data = {"fields": {"Onboarded to Dashboard": True}}
                    ...
                    if response.status_code != 200:
                        app.logger.error(
                            f"Failed to update Airtable onboarded status: {response.text}"
                        )

                except Exception as e:
                    app.logger.error(
                        f"Error updating Airtable onboarded status: {str(e)}"
                    )
                ...
                session.pop("leader_verification", None)
                flash(
                    f"Club successfully verified and updated with official data from the Hack Club directory! Welcome to {club_data['name']}!",
                    "success",
                )
                ...
                session.pop("leader_verification", None)
                flash(
                    "We can't find your club in the Hack Club directory! Please verify your club information again to sync it properly.",
                    "warning",
                )
                return redirect(url_for("club_dashboard", club_id=existing_club.id))
        else:
            ...
            if existing_airtable_club:
                session.pop("leader_verification", None)
                flash(
                    f"This club has already been claimed by another user. If you believe this is an error, please contact support.",
                    "error",
                )
                return redirect(url_for("verify_leader"))
            ...
            if not club_data:
                session.pop("leader_verification", None)
                flash(
                    "We can't find your club in the Hack Club directory! Please verify your club information again.",
                    "error",
                )
                return redirect(url_for("verify_leader"))
            ...
            session.pop("leader_verification", None)
            flash(
                f"Club linked successfully! Welcome to {club_data['name']}!", "success"
            )
...
def join_club_redirect():
    ...
    if is_authenticated():
        ...
        new_membership = ClubMembership(
            user_id=current_user.id, club_id=club.id, role="member"
        )
...
def get_club_chat_messages(club_id):
    try:
        ...
        is_member = (
            ...
            or is_user_co_leader(club, current_user)
            or ClubMembership.query.filter_by(
        ...
        messages = (
            ClubChatMessage.query.filter_by(club_id=club_id)
        ...
        for msg in reversed(messages):  # Reverse to show oldest first
            ...
            msg_dict["can_delete"] = (
                msg.user_id == current_user.id  # Own message
                or club.leader_id == current_user.id  # Leader can delete all
                or club.co_leader_id == current_user.id  # Co-leader can delete all
            )
...
def send_club_chat_message(club_id):
    try:
        ...
        if message_content:
            if (
                PROFANITY_CHECK_AVAILABLE
                and profanity_check.predict([message_content])[0] == 1
            ):
...
def upload_chat_image(club_id):
    try:
        ...
        try:
            ...
            app.logger.info(
                f"Prepared chat image for CDN upload: {temp_filename} ({len(image_data)} bytes)"
            )
        ...
        try:
            ...
            if cdn_response.status_code != 200:
                app.logger.error(
                    f"CDN upload failed: {cdn_response.status_code} - {cdn_response.text}"
                )
            ...
            if "files" in cdn_data and len(cdn_data["files"]) > 0:
                ...
                app.logger.info(
                    f"Returning successful upload response with URL: {uploaded_url}"
                )
                return jsonify({"success": True, "image_url": uploaded_url}), 200
            else:
...
def delete_club_chat_message(club_id, message_id):
    try:
        ...
        can_delete = (
            message.user_id == current_user.id  # Own message
            or club.leader_id == current_user.id  # Leader can delete all
            or is_user_co_leader(club, current_user)  # Co-leader can delete all
            or is_admin_access  # Admin can delete all
        )
...
def get_attendance_sessions(club_id):
    try:
        ...
        sessions = (
            AttendanceSession.query.filter_by(club_id=club_id)
...
def create_attendance_session(club_id):
    try:
        ...
        club = Club.query.get_or_404(club_id)
        if club.leader_id != current_user.id and not is_user_co_leader(
            club, current_user
        ):
        ...
        if max_attendance == "" or max_attendance is None:
            ...
            try:
                ...
                return jsonify(
                    {
                        "error": "Invalid max_attendance value. Must be a number or empty."
                    }
                ), 400
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/sessions/<int:session_id>", methods=["GET"]
)
@login_required
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/sessions/<int:session_id>/members",
    methods=["POST"],
)
@login_required
def add_member_to_session(club_id, session_id):
    try:
        ...
        is_club_member = (
            club.leader_id == user_id
            or is_user_co_leader(club, User.query.get(user_id))
            or ClubMembership.query.filter_by(user_id=user_id, club_id=club_id).first()
        )
        ...
        return jsonify(
            {"success": True, "attendance": attendance_record.to_dict()}
        ), 201
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/sessions/<int:session_id>/attendance",
    methods=["GET"],
)
@login_required
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/sessions/<int:session_id>/guests",
    methods=["GET"],
)
@login_required
def get_session_guests(club_id, session_id):
    try:
        ...
        return jsonify(
            {"success": True, "guests": [guest.to_dict() for guest in guests]}
        ), 200
...
def add_guest_to_session(club_id, session_id):
    try:
        ...
        return jsonify({"success": True, "guest": guest.to_dict()}), 201
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/records/<int:record_id>", methods=["PUT"]
)
@login_required
def update_attendance_record(club_id, record_id):
    try:
        ...
        return jsonify({"success": True, "attendance": record.to_dict()}), 200
...
def get_attendance_reports(club_id):
    try:
        ...
        report_type = request.args.get(
            "type", "summary"
        )  # summary, member_detail, session_detail
        ...
        if report_type == "summary":
            ...
            total_attendance = sum(
                session.get_attendance_count() for session in sessions
            )
            ...
            for member_id in member_stats:
                ...
                stats["attendance_rate"] = (
                    round((attended / stats["total_sessions"]) * 100, 1)
            ...
            report_data = {
                "summary": {
                    ...
                    "average_attendance": round(total_attendance / total_sessions, 1)
                    if total_sessions > 0
                    else 0,
                    "date_range": {
                        "start": sessions[-1].session_date.isoformat()
                        if sessions
                        else None,
                        "end": sessions[0].session_date.isoformat()
                        if sessions
                        else None,
                    },
                },
            ...
            if not member_id:
                return jsonify(
                    {"error": "member_id is required for member_detail report"}
                ), 400
            ...
            for session in sessions:
                record = AttendanceRecord.query.filter_by(
                    session_id=session.id, user_id=member_id
                ).first()
            ...
            if not session_id:
                return jsonify(
                    {"error": "session_id is required for session_detail report"}
                ), 400
            ...
            report_data = {
                ...
                "statistics": {
                    ...
                    "absent": len(
                        [r for r in attendance_records if r.status == "absent"]
                    ),
                    "excused": len(
                        [r for r in attendance_records if r.status == "excused"]
                    ),
                },
            }
        ...
        return jsonify(
            {"success": True, "report_type": report_type, "data": report_data}
        ), 200
                ...
                export_data.append(
                    {
                        ...
                        "status": "present",
                        "check_in_time": guest.check_in_time.isoformat()
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/records/<int:record_id>", methods=["DELETE"]
)
@login_required
def delete_attendance_record(club_id, record_id):
    try:
        ...
        if club.leader_id != current_user.id and not is_user_co_leader(
            ...
            return jsonify(
                {"error": "Only club leaders can delete attendance records"}
            ), 403
...
def update_guest(club_id, guest_id):
    try:
        ...
        return jsonify({"success": True, "guest": guest.to_dict()}), 200
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/guests/<int:guest_id>", methods=["DELETE"]
)
@login_required
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/sessions/<int:session_id>", methods=["PUT"]
)
@login_required
def update_attendance_session(club_id, session_id):
    try:
        ...
        if "session_date" in data:
            try:
                session.session_date = datetime.strptime(
                    data["session_date"], "%Y-%m-%d"
                ).date()
            except ValueError:
                return jsonify({"error": "Invalid date format. Use YYYY-MM-DD"}), 400
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/sessions/<int:session_id>", methods=["DELETE"]
)
@login_required
...
@app.route(
    "/api/clubs/<int:club_id>/attendance/sessions/<int:session_id>/notes",
    methods=["PUT"],
)
@login_required
...
def blog_list():
    ...
    featured_posts = (
        BlogPost.query.filter_by(status="published", is_featured=True)
...
def blog_detail(slug):
    ...
    related_posts = (
        BlogPost.query.filter_by(category_id=post.category_id, status="published")
...
def generate_club_join_code(club_id):
    ...
    if not is_leader and not is_co_leader:
        return jsonify(
            {"error": "Only leaders and co-leaders can generate join codes"}
        ), 403
...
def club_posts(club_id):
    ...
    if request.method == "POST":
        ...
        if not is_leader and not is_co_leader:
            return jsonify(
                {"error": "Only club leaders and co-leaders can create posts"}
            ), 403
        ...
        valid, result = validate_input_with_security(
            content, "club_post", current_user, max_length=5000
        )
        if not valid:
        ...
        if is_leader or is_co_leader:
            ...
            markdown_content = sanitize_string(
                result, max_length=5000, allow_html=False
            )
...
def api_get_project_submissions():
    ...
    try:
        ...
        return jsonify({"success": True, "projects": submissions})
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            current_submission = next(
                (s for s in submissions if s["id"] == project_id), None
            )
            current_status = (
                current_submission.get("status") if current_submission else None
            )
        except Exception as e:
        ...
        update_fields = {"Status": new_status, "Decision Reason": decision_reason}

        # Update in Airtable
        success = airtable_service.update_ysws_project_submission(
        ...
        if success:
            ...
            if new_status == "Approved":
                try:
                    ...
                    if submission:
                        ...
                        grant_amount_raw = submission.get(
                            "grantAmount"
                        )  # This already prioritizes override amount
                        ...
                        if submitter_email and grant_amount_raw:
                            ...
                            try:
                                ...
                                grant_amount_str = re.sub(
                                    r"[^\d.-]", "", grant_amount_str
                                )
                                ...
                                if grant_amount_str:
                                    ...
                                    if grant_amount > 0:
                                        ...
                                        submitter = User.query.filter_by(
                                        ...
                                        if submitter:
                                            ...
                                            if submitter.led_clubs:
                                                ...
                                                club = submitter.club_memberships[
                                                    0
                                                ].club
                                            ...
                                            if club:
                                                ...
                                                try:
                                                    ...
                                                    if success:
                                                        app.logger.info(
                                                            f"Transaction recorded for project grant: {int(grant_amount * 100)} tokens credited"
                                                        )

                                                        # Add 100 tokens to piggy bank for approved project
                                                        try:
                                                            club.piggy_bank_tokens = (
                                                                club.piggy_bank_tokens
                                                                or 0
                                                            ) + 100
                                                            db.session.commit()
                                                            app.logger.info(
                                                                f"Added 100 tokens to piggy bank for club '{club.name}' (ID: {club.id}) - Total piggy bank: {club.piggy_bank_tokens}"
                                                            )
                                                            ...
                                                            try:
                                                                ...
                                                                if success:
                                                                    app.logger.info(
                                                                        f"Piggy bank transaction recorded: 100 tokens credited"
                                                                    )
                                                            except (
                                                                Exception
                                                            ) as piggy_tx_error:
                                                                app.logger.error(
                                                                    f"Failed to record piggy bank transaction: {str(piggy_tx_error)}"
                                                                )
                                                        except Exception as piggy_error:
                                                            db.session.rollback()
                                                            app.logger.error(
                                                                f"Failed to add tokens to piggy bank: {str(piggy_error)}"
                                                            )
                                                    else:
                                                        app.logger.error(
                                                            f"Failed to record transaction for project grant: {tx_result}"
                                                        )
                                                except Exception as tx_error:
                                                    app.logger.error(
                                                        f"Exception while recording project grant transaction: {str(tx_error)}"
                                                    )

                                                app.logger.info(
                                                    f"Added ${grant_amount} to club '{club.name}' (ID: {club.id}) for approved project {project_id}"
                                                )
                                                ...
                                                try:
                                                    project_submission = (
                                                        ProjectSubmission(
                                                            ...
                                                            club_id=club.id,
                                                            project_name=submission.get(
                                                                "hackatimeProject",
                                                                "Unknown Project",
                                                            ),
                                                            project_url=submission.get(
                                                                "codeUrl"
                                                            )
                                                            or submission.get(
                                                                "playableUrl"
                                                    ...
                                                    update_quest_progress(
                                                        club.id, "member_projects", 1
                                                    )

                                                    app.logger.info(
                                                        f"Logged project submission for user {submitter.username} in club {club.name}"
                                                    )
                                                except Exception as proj_log_error:
                                                    app.logger.error(
                                                        f"Failed to log project submission: {str(proj_log_error)}"
                                                    )
                                                    db.session.rollback()
                                            else:
                                                app.logger.warning(
                                                    f"User {submitter_email} is not associated with any club"
                                                )
                                        else:
                                            app.logger.warning(
                                                f"Submitter {submitter_email} not found in system"
                                            )
                            except (ValueError, TypeError) as e:
                                app.logger.error(
                                    f"Error parsing grant amount '{grant_amount_raw}': {str(e)}"
                                )
                        else:
                            app.logger.warning(
                                f"Missing email or grant amount for project {project_id}"
                            )
                    else:
                        app.logger.warning(f"Project submission {project_id} not found")
                except Exception as e:
                    app.logger.error(
                        f"Error adding grant amount to club balance: {str(e)}"
                    )
                                        ...
                                        if submitter:
                                            ...
                                            if club:
                                                ...
                                                try:
                                                    ...
                                                    if success:
                                                        app.logger.info(
                                                            f"Transaction recorded for project grant revocation: {int(grant_amount * 100)} tokens deducted"
                                                        )
                                                        ...
                                                        try:
                                                            if (
                                                                ...
                                                                db.session.commit()
                                                                app.logger.info(
                                                                    f"Deducted 100 tokens from piggy bank for club '{club.name}' (ID: {club.id}) - Total piggy bank: {club.piggy_bank_tokens}"
                                                                )
                                                                ...
                                                                try:
                                                                    ...
                                                                    if success:
                                                                        app.logger.info(
                                                                            f"Piggy bank transaction recorded: 100 tokens deducted"
                                                                        )
                                                                except (
                                                                    Exception
                                                                ) as piggy_tx_error:
                                                                ...
                                                                app.logger.warning(
                                                                    f"Club '{club.name}' has insufficient piggy bank tokens ({club.piggy_bank_tokens}) to deduct 100 tokens"
                                                                )
                                                        except Exception as piggy_error:
                                                            db.session.rollback()
                                                            app.logger.error(
                                                                f"Failed to deduct tokens from piggy bank: {str(piggy_error)}"
                                                            )
                                                    else:
                                                        app.logger.error(
                                                            f"Failed to record transaction for project grant revocation: {tx_result}"
                                                        )
                                                except Exception as tx_error:
                                                    app.logger.error(
                                                        f"Exception while recording project grant revocation transaction: {str(tx_error)}"
                                                    )

                                                app.logger.info(
                                                    f"Revoked ${grant_amount} from club '{club.name}' (ID: {club.id}) for project {project_id} status change"
                                                )
                            except (ValueError, TypeError) as e:
                                app.logger.error(
                                    f"Error parsing grant amount for revocation '{grant_amount_raw}': {str(e)}"
                                )
                except Exception as e:
                    app.logger.error(
                        f"Error revoking grant amount from club balance: {str(e)}"
                    )

            # Log the review action
            app.logger.info(
                f"{('Admin' if current_user.is_admin else 'Reviewer')} {current_user.username} updated project {project_id}: status={new_status}"
            )
            ...
            try:
                ...
                try:
                    ...
                    app.logger.info(
                        f"Audit log created successfully for project review: {log_entry.id}"
                    )

                except Exception as audit_error:
                    audit_session.rollback()
                    app.logger.error(
                        f"Failed to create audit log in separate session: {str(audit_error)}"
                    )
                finally:
                    audit_session.close()
            ...
            return jsonify(
                {"success": True, "message": "Project review updated successfully"}
            )
        else:
            return jsonify(
                {"success": False, "error": "Failed to update project in Airtable"}
            ), 500
...
def api_grant_override(project_id):
    ...
    try:
        ...
        if grant_amount is None or not override_reason:
            return jsonify(
                {"error": "Grant amount and override reason are required"}
            ), 400
        ...
        if success:
            ...
            current_user = get_current_user()
            app.logger.info(
                f"Admin {current_user.username} overrode grant amount for project {project_id}: ${grant_amount} - {override_reason}"
            )
            ...
            return jsonify(
                {"success": False, "error": "Failed to update grant amount in Airtable"}
            ), 500
...
def api_delete_project(project_id):
    ...
    try:
        ...
        if success:
            ...
            current_user = get_current_user()
            app.logger.info(
                f"Admin {current_user.username} deleted project submission {project_id}"
            )
            ...
            return jsonify(
                {"success": True, "message": "Project submission deleted successfully"}
            )
        else:
            return jsonify(
                {"success": False, "error": "Failed to delete project from Airtable"}
            ), 500
...
def api_update_order_status(order_id):
    ...
    try:
        ...
        if result:
            # Log the review action
            app.logger.info(
                f"{'Admin' if current_user.is_admin else 'Reviewer'} {current_user.username} updated order {order_id} status to {status}"
            )
...
def api_delete_order(order_id):
    ...
    try:
        ...
        if result:
            ...
            app.logger.warning(
                f"Admin {current_user.username} deleted order {order_id}"
            )
...
def api_refund_order(order_id):
    ...
    try:
            ...
            if club:
                ...
                try:
                    success, tx_result = create_club_transaction(
                        ...
                        amount=int(
                            refund_amount * 100
                        ),  # Convert to tokens (positive for credit)
                    ...
                    if success:
                        app.logger.info(
                            f"Transaction recorded for refund: {int(refund_amount * 100)} tokens credited"
                        )
                    else:
                        app.logger.error(
                            f"Failed to record transaction for refund: {tx_result}"
                        )
                except Exception as tx_error:
                    app.logger.error(
                        f"Exception while recording refund transaction: {str(tx_error)}"
                    )

                app.logger.info(
                    f"Refunded {int(refund_amount * 100)} tokens to club '{club_name}' balance (now {club.tokens} tokens)"
                )
        ...
        if result:
            ...
            app.logger.warning(
                f"Admin {current_user.username} refunded order {order_id} - ${refund_amount} credited to {club_name}"
            )
...
def admin_users_by_ip():
    ...
    all_users = list(
        {
            user.id: user
            for user in users_by_reg_ip + users_by_login_ip + users_by_history
        }.values()
    )
    ...
    return jsonify(
        {"ip_address": ip_address, "users": users_data, "total_users": len(users_data)}
    )
...
def club_assignments(club_id):
    ...
    if request.method == "POST":
        ...
        if not is_leader and not is_co_leader:
            return jsonify(
                {"error": "Only club leaders and co-leaders can create assignments"}
            ), 403
        ...
        valid, result = validate_input_with_security(
            title, "assignment_title", current_user, max_length=200
        )
        if not valid:
            return jsonify({"error": result}), 403
        ...
        valid, result = validate_input_with_security(
            description, "assignment_description", current_user, max_length=5000
        )
        if not valid:
            return jsonify({"error": result}), 403
    ...
    assignments = (
        ClubAssignment.query.filter_by(club_id=club_id)
        .order_by(ClubAssignment.created_at.desc())
        .all()
    )
    assignments_data = [
        {
...
def club_meetings(club_id):
    ...
    if request.method == "POST":
        ...
        if not is_leader and not is_co_leader:
            return jsonify(
                {"error": "Only club leaders and co-leaders can schedule meetings"}
            ), 403
        ...
        for field in text_fields:
            if field in data and data[field]:
                valid, result = validate_input_with_security(
                    data[field], f"meeting_{field}", current_user
                )
                if not valid:
                    return jsonify({"error": result}), 403
    ...
    meetings = (
        ClubMeeting.query.filter_by(club_id=club_id)
        .order_by(ClubMeeting.meeting_date.desc())
        .all()
    )
    meetings_data = [
        {
...
@api_route(
    "/api/clubs/<int:club_id>/meetings/<int:meeting_id>", methods=["PUT", "DELETE"]
)
@login_required
@limiter.limit("200 per hour")
...
def submit_project(club_id):
    ...
    for field in text_fields:
        if field in data and data[field]:
            valid, result = validate_input_with_security(
                data[field], f"project_{field}", current_user
            )
            if not valid:
                return jsonify({"error": result}), 403
            data[field] = result
    ...
    if member_id and member_id != str(current_user.id):
        ...
        selected_is_co_leader = (
            club.co_leader_id == selected_user.id if club.co_leader_id else False
        )
        selected_is_member = ClubMembership.query.filter_by(
            club_id=club_id, user_id=selected_user.id
        ).first()

        if (
            not selected_is_leader
            and not selected_is_co_leader
            and not selected_is_member
        ):
    ...
    app.logger.info(
        f"Project submission for club {club.name}: {submission_data.get('project_name')}"
    )

    # Log the submission for tracking
    app.logger.info(
        f"Project submission by user {current_user.id} for club {club_id}: {submission_data}"
    )
    ...
    try:
        ...
        if hasattr(airtable_service, "submit_project_data"):
            ...
            if airtable_result:
                app.logger.info(
                    f"AIRTABLE: Successfully submitted project to Airtable: {airtable_result}"
                )
            else:
...
def get_club_transactions(club_id):
    ...
    transactions_data = [
        transaction.to_dict() for transaction in transactions_pagination.items
    ]
    ...
    return jsonify(
        {
            ...
            "club": {"id": club.id, "name": club.name, "current_balance": club.tokens},
        }
    )
...
def club_pizza_grants(club_id):
    ...
    if request.method == "POST":
        ...
        if not is_in_person:
            return jsonify(
                {
                    "error": "Project submissions must be from in-person meetings only. Virtual meetings are not eligible for grants."
                }
            ), 400
        ...
        if member_count < 3:
            return jsonify(
                {
                    "error": f"Your club must have at least 3 members to submit for grants. Current members: {member_count}"
                }
            ), 400
        ...
        app.logger.info(
            f"Pizza grant submission data: project_name={submission_data.get('project_name')}"
        )
        app.logger.info(
            f"Screenshot URL received: {submission_data.get('screenshot_url')}"
        )
    ...
    try:
        ...
        club_submissions = [
            ...
            if s.get("club_name", "").lower() == club.name.lower()
        ]
...
def club_projects(club_id):
    ...
    projects = (
        ClubProject.query.filter_by(club_id=club_id)
        .order_by(ClubProject.updated_at.desc())
        .all()
    )
    projects_data = [
        {
            ...
            "owner": {"id": project.user.id, "username": project.user.username},
        }
...
def club_resources(club_id):
    ...
    if request.method == "POST":
        ...
        if not is_leader and not is_co_leader:
            return jsonify(
                {"error": "Only club leaders and co-leaders can add resources"}
            ), 403
        ...
        for field in resource_fields:
            if field in data and data[field]:
                valid, result = validate_input_with_security(
                    data[field], f"resource_{field}", current_user
                )
                if not valid:
                    return jsonify({"error": result}), 403
                data[field] = sanitize_string(result)
    ...
    resources = (
        ClubResource.query.filter_by(club_id=club_id)
...
@api_route(
    "/api/clubs/<int:club_id>/assignments/<int:assignment_id>", methods=["DELETE"]
)
@login_required
@limiter.limit("200 per hour")
...
@api_route(
    "/api/clubs/<int:club_id>/resources/<int:resource_id>", methods=["PUT", "DELETE"]
)
@login_required
@limiter.limit("200 per hour")
...
def get_club_members(club_id):
    ...
    members_data.sort(
        key=lambda x: (role_priority.get(x["role"], 4), x["username"].lower())
    )
...
def remove_club_member(club_id, user_id):
    ...
    is_authorized, role = verify_club_leadership(
        club, current_user, require_leader_only=False
    )

    if not is_authorized:
        app.logger.warning(
            f"Unauthorized member removal attempt: User {current_user.id} tried to remove member {user_id} from club {club_id}"
        )
        return jsonify(
            {
                "error": "Unauthorized: Only club leaders and co-leaders can remove members"
            }
        ), 403
    ...
    try:
        ...
        db.session.commit()
        app.logger.info(
            f"Member removed: User {current_user.id} ({role}) removed member {user_id} from club {club_id}"
        )
...
def make_co_leader(club_id):
    ...
    is_authorized, role = verify_club_leadership(
        club, current_user, require_leader_only=True
    )

    if not is_authorized:
        app.logger.warning(
            f"Unauthorized co-leader management attempt: User {current_user.id} tried to manage co-leader for club {club_id}"
        )
        return jsonify(
            {"error": "Unauthorized: Only club leaders can manage co-leaders"}
        ), 403
    ...
    if "step" in data and data["step"] == "verify_email":
        ...
        # Verify the email code
        is_code_valid = airtable_service.verify_email_code(
            club.leader.email, verification_code
        )
    ...
    if request.method == "DELETE":
        ...
        try:
            ...
            db.session.commit()
            return jsonify(
                {"success": True, "message": "Co-leader removed successfully"}
            )
        ...
        try:
            ...
            if membership:
                ...
                new_membership = ClubMembership(
                    user_id=user_id, club_id=club_id, role="co-leader"
                )
                db.session.add(new_membership)

            db.session.commit()
            return jsonify(
                {"success": True, "message": "User promoted to co-leader successfully"}
            )
...
def remove_co_leader(club_id):
    ...
    if not is_authorized:
        app.logger.warning(
            f"Unauthorized co-leader removal attempt: User {current_user.id} tried to remove co-leader from club {club_id}"
        )
        return jsonify(
            {"error": "Unauthorized: Only club leaders can remove co-leaders"}
        ), 403
    ...
    try:
        ...
        # Update membership role back to member
        membership = ClubMembership.query.filter_by(
            club_id=club_id, user_id=co_leader_id
        ).first()
        if membership:
            membership.role = "member"
...
def update_club_settings(club_id):
    ...
    if not is_authorized:
        app.logger.warning(
            f"Unauthorized settings update attempt: User {current_user.id} tried to update settings for club {club_id}"
        )
        return jsonify(
            {
                "error": "Unauthorized: Only club leaders and co-leaders can update settings"
            }
        ), 403
    ...
    requires_verification = any(
        key in data for key in ["name", "location", "description"]
    )
    ...
    if "name" in data:
        ...
        # Security validation with auto-suspend
        valid, result = validate_input_with_security(
            data["name"], "club_name", current_user, max_length=100
        )
        if not valid:
            return jsonify({"error": result}), 403
    ...
    if "description" in data:
        ...
        # Security validation with auto-suspend
        valid, result = validate_input_with_security(
            data["description"], "club_description", current_user, max_length=1000
        )
        if not valid:
            return jsonify({"error": result}), 403
    ...
    if "location" in data:
        ...
        # Security validation with auto-suspend
        valid, result = validate_input_with_security(
            data["location"], "club_location", current_user, max_length=255
        )
        if not valid:
            return jsonify({"error": result}), 403
    ...
    if airtable_data and airtable_data.get("airtable_id"):
        try:
            ...
            if airtable_fields:
                ...
                response = requests.patch(
                    update_url, headers=airtable_service.headers, json=payload
                )

                if response.status_code == 200:
                    app.logger.info(
                        f"Successfully synced club {club_id} changes to Airtable"
                    )
                else:
                    app.logger.warning(
                        f"Failed to sync club {club_id} to Airtable: {response.status_code} - {response.text}"
                    )
        except Exception as e:
...
def update_club_email(club_id):
    ...
    if current_user.id != club.leader_id:
        app.logger.warning(
            f"Unauthorized email update attempt: User {current_user.id} tried to update email for club {club_id}"
        )
        return jsonify(
            {"error": "Unauthorized: Only club leaders can update email"}
        ), 403
    ...
    try:
        app.logger.info(
            f"Updating email for club {club_id} from {club.leader.email} to {current_user.email}"
        )
        ...
        airtable_fields = {"Current Leaders' Emails": current_user.email}
        ...
        if response.status_code == 200:
            app.logger.info(
                f"Successfully updated club {club_id} email in Airtable to {current_user.email}"
            )
            return jsonify(
                {
                    "success": True,
            ...
            app.logger.error(
                f"Failed to update club {club_id} email in Airtable: {response.status_code} - {response.text}"
            )
            return jsonify(
                {"error": "Failed to update email in Hack Club records"}
            ), 500
...
def manage_club_background(club_id):
    ...
    if not is_authorized:
        app.logger.warning(
            f"Unauthorized background management attempt: User {current_user.id} tried to manage background for club {club_id}"
        )
        return jsonify(
            {
                "error": "Unauthorized: Only club leaders and co-leaders can manage background"
            }
        ), 403
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not cdn_token:
                app.logger.error(
                    "HACKCLUB_CDN_TOKEN not found in environment variables"
                )
            ...
            # Create public URL for the temp file that CDN can access
            temp_filename = (
                f"club_background_{club_id}_{uuid.uuid4().hex[:8]}.{file_ext}"
            )
            temp_upload_dir = os.path.join(app.root_path, "static", "temp")
            os.makedirs(temp_upload_dir, exist_ok=True)
            temp_public_path = os.path.join(temp_upload_dir, temp_filename)
            ...
            app.logger.info(
                f"Prepared club background image for CDN upload: {temp_filename} ({len(image_data)} bytes)"
            )
        ...
        try:
            ...
            if "files" in cdn_data and len(cdn_data["files"]) > 0:
                ...
                try:
                    ...
                    app.logger.warning(
                        f"Failed to clean up temp file {temp_public_path}: {str(e)}"
                    )
                ...
                try:
                    ...
                    db.session.rollback()
                    app.logger.error(
                        f"Error saving background URL to database: {str(e)}"
                    )
        ...
        try:
            if "blur" in data:
                ...
                if (
                    not isinstance(blur_value, (int, float))
                    or blur_value < 0
                    or blur_value > 100
                ):
                    return jsonify(
                        {"error": "Blur value must be between 0 and 100"}
                    ), 400
...
def transfer_leadership(club_id):
    ...
    if not is_authorized:
        app.logger.warning(
            f"Unauthorized leadership transfer attempt: User {current_user.id} tried to transfer leadership for club {club_id}"
        )
        return jsonify(
            {"error": "Unauthorized: Only club leaders can transfer leadership"}
        ), 403
    ...
    if "step" in data and data["step"] == "send_verification":
        app.logger.info(
            f"Leadership transfer: Sending verification code to {club.leader.email} for club {club_id}"
        )
        verification_code = airtable_service.send_email_verification(club.leader.email)

        if verification_code:
            app.logger.info(
                f"Leadership transfer: Verification code successfully sent for {club.leader.email}"
            )
            return jsonify(
                {
                    "success": True,
            ...
            app.logger.error(
                f"Leadership transfer: Failed to send verification code to {club.leader.email}"
            )
            return jsonify(
                {
                    "error": "Failed to send verification code. This may be due to a network timeout. Please try again in a moment."
                }
            ), 500
    ...
    membership = ClubMembership.query.filter_by(
        club_id=club_id, user_id=new_leader_id
    ).first()
    if not membership:
    ...
    if club.co_leader_id == new_leader_id:
        return jsonify(
            {
                "error": "Cannot transfer leadership to current co-leader. Remove co-leader first or choose someone else."
            }
        ), 400
    ...
    try:
        ...
        app.logger.info(
            f"Leadership transferred: Club {club_id} from user {old_leader_id} to user {new_leader_id}"
        )
...
def club_grant_submissions(club_id):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            app.logger.error(
                f"Error fetching grant submissions for club {club_id}: {str(e)}"
            )
            return jsonify({"submissions": []})
...
def gallery_posts():
    if request.method == "POST":
        ...
        if not club_id or not title or not description:
            return jsonify(
                {"error": "Club ID, title, and description are required"}
            ), 400
        ...
        # Security validation
        valid, result = validate_input_with_security(
            title, "gallery_title", current_user, max_length=200
        )
        if not valid:
            return jsonify({"error": result}), 403
        title = result
        ...
        valid, result = validate_input_with_security(
            description, "gallery_description", current_user, max_length=2000
        )
        if not valid:
            return jsonify({"error": result}), 403
        description = result
        ...
        if current_user.is_admin and custom_club_name:
            valid, result = validate_input_with_security(
                custom_club_name, "custom_club_name", current_user, max_length=100
            )
            if not valid:
                return jsonify({"error": result}), 403
        ...
        app.logger.info(
            f"Gallery post created: ID={post.id}, title='{title}', club_id={club_id}, images={len(images)}"
        )
        ...
        try:
            ...
            if airtable_success:
                app.logger.info(
                    f"Gallery post {post.id} successfully logged to Airtable"
                )
            else:
                app.logger.warning(f"Failed to log gallery post {post.id} to Airtable")
        except Exception as e:
            app.logger.error(
                f"Exception logging gallery post {post.id} to Airtable: {str(e)}"
            )
        ...
        return jsonify(
            {"message": "Gallery post created successfully", "post_id": post.id}
        )
        ...
        for post in posts:
            try:
                ...
                if not club or not user:
                    app.logger.warning(
                        f"Skipping post {post.id}: missing club ({club}) or user ({user})"
                    )
                    continue
                ...
                if post.description.startswith("[CUSTOM_CLUB:"):
                    ...
                    try:
                        ...
                        if end_idx != -1:
                            custom_club_name = post.description[
                                13:end_idx
                            ]  # Skip '[CUSTOM_CLUB:'
                ...
                posts_data.append(post_data)
                app.logger.debug(
                    f"Gallery post {post.id}: '{post.title}' by {user.username} from {club.name}, {len(post.get_images())} images"
                )
...
def delete_gallery_post(post_id):
    ...
    try:
        ...
        if club and club.tokens >= 100:
            ...
            if not success:
                app.logger.warning(
                    f"Failed to deduct tokens for gallery post deletion: {error_msg}"
                )
        ...
        return jsonify(
            {"success": True, "message": "Gallery post deleted successfully"}
        )
...
def club_purchase_requests(club_id):
    ...
    if request.method == "POST":
        ...
        if not is_leader and not is_co_leader:
            return jsonify(
                {
                    "error": "Only club leaders and co-leaders can submit purchase requests"
                }
            ), 403
        ...
        if amount > (club.tokens / 100.0):
            return jsonify(
                {"error": f"Amount cannot exceed club balance of {club.tokens} tokens"}
            ), 400
        ...
        if result:
            ...
            try:
                ...
                if success:
                    app.logger.info(
                        f"Transaction recorded for purchase request: {int(amount * 100)} tokens deducted"
                    )
                else:
                    app.logger.error(
                        f"Failed to record transaction for purchase request: {tx_result}"
                    )
            except Exception as tx_error:
                app.logger.error(
                    f"Exception while recording purchase request transaction: {str(tx_error)}"
                )
...
def upload_screenshot():
    ...
    app.logger.info(
        f"File received: {file.filename}, content_type: {file.content_type}"
    )
    ...
    if file.content_type not in allowed_mime_types:
        ...
        return jsonify(
            {
                "success": False,
                "error": "Invalid image format. Only JPEG, PNG, GIF, and WebP allowed.",
            }
        ), 400
...
def upload_images():
    ...
    try:
        ...
        if not isinstance(base64_images, list):
            return jsonify(
                {"success": False, "error": "Images must be provided as an array"}
            ), 400
        ...
        if len(base64_images) > max_images:
            return jsonify(
                {"success": False, "error": f"Maximum {max_images} images allowed"}
            ), 400
        ...
        if not cdn_token:
            app.logger.error("HACKCLUB_CDN_TOKEN not configured")
            return jsonify(
                {"success": False, "error": "CDN service not configured"}
            ), 500
        ...
        for i, base64_data in enumerate(base64_images):
            ...
            try:
                ...
                if len(image_data) > max_size:
                    app.logger.warning(
                        f"Skipping image {i}: too large ({len(image_data)} bytes, max {max_size})"
                    )
                    continue
                ...
                app.logger.info(
                    f"Prepared image {i} for CDN upload: {temp_filename} ({len(image_data)} bytes)"
                )
        ...
        try:
            ...
            if cdn_response.status_code != 200:
                ...
                return jsonify(
                    {"success": False, "error": "Failed to upload to CDN"}
                ), 500
            ...
            if "files" in cdn_data:
                uploaded_urls = [
                    file_info["deployedUrl"] for file_info in cdn_data["files"]
                ]
            else:
                app.logger.error(f"Unexpected CDN response format: {cdn_data}")
                return jsonify(
                    {"success": False, "error": "Unexpected CDN response format"}
                ), 500

        except requests.RequestException as e:
            app.logger.error(f"Error uploading to CDN: {str(e)}")
            return jsonify(
                {"success": False, "error": "Failed to connect to CDN service"}
            ), 500
            ...
            for temp_url in cdn_url_list:
                ...
                try:
                    ...
                    app.logger.warning(
                        f"Failed to clean up temp file {temp_file_path}: {str(e)}"
                    )
        ...
        if not uploaded_urls:
            return jsonify(
                {
                    "success": False,
                    "error": "No images were successfully uploaded to CDN",
                }
            ), 400

        app.logger.info(
            f"Successfully uploaded {len(uploaded_urls)} images to HackClub CDN (bulk_upload={bulk_upload})"
        )
...
def upload_blog_images():
    ...
    if not current_user.is_admin and not current_user.is_reviewer:
        return jsonify(
            {"error": "Only admins and reviewers can upload blog images"}
        ), 403
    ...
    try:
        ...
        for i, image_data in enumerate(images):
            try:
                ...
                try:
                    ...
                    app.logger.error(
                        f"Base64 decode error for image {i}: {str(decode_error)}"
                    )
                    continue
        ...
        try:
            ...
            if cdn_response.status_code != 200:
                app.logger.error(
                    f"CDN upload failed with status {cdn_response.status_code}: {cdn_response.text}"
                )
            ...
            for temp_file in temp_files:
                try:
                    ...
                    app.logger.error(
                        f"Error cleaning up temp file {temp_file}: {str(e)}"
                    )
...
def get_hackatime_projects(user_id):
    current_user = get_current_user()
    app.logger.info(
        f"get_hackatime_projects: Request from user {current_user.username} (ID: {current_user.id}) for user_id {user_id}"
    )

    # Only allow users to access their own data or club leaders to access member data
    if user_id != current_user.id:
        app.logger.info(
            f"get_hackatime_projects: Cross-user access requested, checking leader permissions"
        )
        is_leader = False
        led_clubs = Club.query.filter_by(leader_id=current_user.id).all()
        app.logger.info(
            f"get_hackatime_projects: Current user leads {len(led_clubs)} clubs"
        )
        ...
        for club in led_clubs:
            ...
            if membership or club.leader_id == user_id:
                is_leader = True
                app.logger.info(
                    f"get_hackatime_projects: Access granted - user is member/leader of club {club.id}"
                )
                break

        if not is_leader:
            app.logger.warning(
                f"get_hackatime_projects: Access denied - user {current_user.id} is not leader of user {user_id}"
            )
            return jsonify({"error": "Unauthorized"}), 403

    user = User.query.get_or_404(user_id)
    app.logger.info(
        f"get_hackatime_projects: Target user: {user.username} (ID: {user.id})"
    )

    if not user.hackatime_api_key:
        app.logger.warning(
            f"get_hackatime_projects: User {user.username} has no Hackatime API key configured"
        )
        return jsonify({"error": "User has not configured Hackatime API key"}), 400

    app.logger.info(
        f"get_hackatime_projects: User {user.username} has API key configured, fetching projects"
    )
    projects = hackatime_service.get_user_projects(user.hackatime_api_key)

    app.logger.info(
        f"get_hackatime_projects: Returning {len(projects)} projects for user {user.username}"
    )
    response_data = {"username": user.username, "projects": projects}
...
def admin_get_users():
    ...
    if sort == "created_at-desc":
        ...
        query = query.order_by(
            case((User.is_suspended == True, 1), else_=0).desc(), User.created_at.desc()
        )
        ...
        query = query.order_by(
            case((User.is_suspended == True, 1), else_=0).asc(), User.created_at.desc()
        )
    else:
...
def admin_users_group_by_ip():
    ...
    sort_by = request.args.get(
        "sort", "users-desc"
    )  # users-desc, users-asc, ip-asc, ip-desc
    ...
    login_ip_query = (
        ...
        .filter(
            ...
            ~User.last_login_ip.in_(
                db.session.query(User.registration_ip).filter(
    ...
    combined_query = union_all(reg_ip_query.statement, login_ip_query.statement).alias(
        "ip_groups"
    )
    ...
    if sort_by == "users-desc":
        ip_groups_query = ip_groups_query.order_by(
            func.sum(combined_query.c.user_count).desc()
        )
    elif sort_by == "users-asc":
        ip_groups_query = ip_groups_query.order_by(
            func.sum(combined_query.c.user_count).asc()
        )
    elif sort_by == "ip-asc":
    ...
    for ip_row in paginated_ips:
        ...
        users_for_ip = (
            ...
            .filter(db.or_(User.registration_ip == ip, User.last_login_ip == ip))
        ...
        groups.append(
            {
                ...
                "type": " & ".join(ip_types)
                if len(ip_types) > 1
                else (ip_types[0] if ip_types else "Unknown"),
            }
        )
...
def admin_users_group_by_club():
    ...
    sort_by = request.args.get(
        "sort", "users-desc"
    )  # users-desc, users-asc, name-asc, name-desc
    ...
    club_user_counts = (
        db.session.query(
            ...
            (
                ...
                + case((Club.leader_id.isnot(None), 1), else_=0)
            ).label("total_users"),
        )
        .outerjoin(
            ClubMembership,
            db.and_(
                ...
                ClubMembership.user_id != Club.leader_id,
            ),
        )
        .group_by(Club.id, Club.name, Club.join_code, Club.leader_id)
        .having(
            (
                func.count(ClubMembership.user_id)
                + case((Club.leader_id.isnot(None), 1), else_=0)
    ...
    users_without_clubs_count = (
        ...
        .filter(
            ...
            ~User.id.in_(
                db.session.query(ClubMembership.user_id).filter(
                    ClubMembership.user_id.isnot(None)
                )
            ),
        )
        .count()
            ...
            club_users.append(
                {
                    ...
                    "created_at": club.leader.created_at.isoformat()
                    if club.leader.created_at
                    else None,
                    "role": "Leader",
                }
            )
    ...
    if users_without_clubs_count > 0:
        ...
        if should_include_orphans:
            ...
            orphan_users = (
                ...
                .filter(
                    ~User.id.in_(
                        db.session.query(Club.leader_id).filter(
                            Club.leader_id.isnot(None)
                        )
                    ),
                    ~User.id.in_(
                        db.session.query(ClubMembership.user_id).filter(
...
def admin_get_clubs():
    ...
    # Apply pagination
    clubs_paginated = query.paginate(page=page, per_page=per_page, error_out=False)
    ...
    clubs_data = [
        {
            ...
            + (1 if club.leader else 0),  # +1 for leader if exists
...
def admin_manage_user(user_id):
    ...
    if request.method == "DELETE":
        try:
            ...
            app.logger.info(
                f"Admin {current_user.username} deleted user {user.username} (ID: {user_id})"
            )
...
def admin_create_club():
    ...
    if step == "verify_email":
        ...
        # Verify the email code
        is_code_valid = airtable_service.verify_email_code(
            leader_email, verification_code
        )
    ...
    if existing_club:
        return jsonify(
            {"error": f"User is already leading club: {existing_club.name}"}
        ), 400
    ...
    if existing_club:
        return jsonify(
            {"error": f'A club with the name "{filtered_name}" already exists'}
        ), 400
    ...
    try:
        ...
        app.logger.info(
            f"Admin {current_user.username} created club {name} for user {leader.username} (email verified)"
        )
...
def admin_manage_club(club_id):
    ...
    if request.method == "DELETE":
        try:
            ...
            app.logger.info(
                f"Admin {current_user.username} successfully deleted club {club.name} (ID: {club_id})"
            )
    ...
    if request.method == "PUT":
        ...
        if "name" in data:
            ...
            existing_club = Club.query.filter(
                Club.name == filtered_name, Club.id != club_id
            ).first()
            if existing_club:
                return jsonify(
                    {"error": f'A club with the name "{filtered_name}" already exists'}
        ...
        if "balance" in data:
            ...
            if balance_change_usd != 0:
                ...
                try:
                    ...
                    if success:
                        app.logger.info(
                            f"Transaction recorded for admin balance adjustment: {int(balance_change_usd * 100)} tokens"
                        )
                    else:
                        app.logger.error(
                            f"Failed to record transaction for admin balance adjustment: {tx_result}"
                        )
                except Exception as tx_error:
                    app.logger.error(
                        f"Exception while recording admin balance adjustment transaction: {str(tx_error)}"
                    )
...
def admin_transfer_club_leadership(club_id):
    ...
    if current_leader_id == new_leader_id:
        return jsonify(
            {"error": "New leader must be different from current leader"}
        ), 400
    ...
    try:
        ...
        existing_membership = ClubMembership.query.filter_by(
            user_id=new_leader_id, club_id=club_id
        ).first()
        ...
        if current_leader_id:
            old_leader_membership = ClubMembership.query.filter_by(
                user_id=current_leader_id, club_id=club_id
            ).first()
        ...
        app.logger.info(
            f"Admin {current_user.username} transferred leadership of club '{club.name}' (ID: {club_id}) from {old_leader_username} to {new_leader.username}"
        )
        ...
        return jsonify(
            {"error": "Failed to transfer leadership. Please try again."}
        ), 500
...
def admin_search_clubs():
    ...
    search_term = f"%{query}%"
    clubs = (
        Club.query.join(User, Club.leader_id == User.id)
        .filter(
            db.or_(
                Club.name.ilike(search_term),
                Club.location.ilike(search_term),
...
def admin_login_as_user(user_id):
    ...
    if not current_user.is_admin:
        app.logger.warning(
            f"Non-admin user {current_user.id} attempted to use admin login-as-user"
        )
        return jsonify({"error": "Admin access required"}), 403
    ...
    if user.email == "ethan@hackclub.com" or user.is_admin:
        app.logger.warning(
            f"Admin {current_user.id} attempted to login as admin user {user.id}"
        )
        return jsonify({"error": "Cannot login as admin users"}), 400

    # Additional security: log the action with full context
    app.logger.warning(
        f"ADMIN IMPERSONATION: Admin {current_user.username} (ID: {current_user.id}) logging in as user {user.username} (ID: {user.id}) from IP: {request.remote_addr}"
    )
...
def admin_suspend_user(user_id):
    ...
    try:
            ...
            if led_clubs and (suspend_club_members or suspend_club):
                for club in led_clubs:
                    ...
                    if suspend_club_members:
                        ...
                        for membership in club.members:
                            if (
                                membership.user.email != "ethan@hackclub.com"
                            ):  # Don't suspend super admin
                                membership.user.is_suspended = True
                                actions_taken.append(
                                    f"Club member {membership.user.username} suspended"
                                )
        ...
        app.logger.info(
            f"Admin {current_user.username} {action_verb} user {user.username} (ID: {user.id}). Actions: {'; '.join(actions_taken)}"
        )
...
def admin_sync_clubs_airtable():
    ...
    try:
        ...
        if result["success"]:
            ...
            app.logger.info(
                f"Admin {current_user.username} synced clubs with Airtable: {message}"
            )
            return jsonify(
                {
                    "success": True,
        ...
        return jsonify(
            {"success": False, "error": "Failed to sync clubs with Airtable"}
        ), 500
...
def admin_preview_airtable_clubs():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": "Failed to fetch clubs from Airtable"}
        ), 500
...
def admin_get_leaderboard_exclusions():
    ...
    try:
        ...
        exclusions = (
            db.session.query(LeaderboardExclusion)
            .join(Club)
            .join(User, LeaderboardExclusion.excluded_by == User.id)
            .filter(LeaderboardExclusion.leaderboard_type == leaderboard_type)
            .all()
        )
        ...
        return jsonify({"success": True, "exclusions": exclusions_data})

    except Exception as e:
        app.logger.error(f"Error fetching leaderboard exclusions: {str(e)}")
        return jsonify({"success": False, "error": "Failed to fetch exclusions"}), 500
...
def admin_add_leaderboard_exclusion():
    ...
    try:
        ...
        existing = LeaderboardExclusion.query.filter_by(
            club_id=club_id, leaderboard_type=leaderboard_type
        ).first()

        if existing:
            return jsonify(
                {"error": "Club already excluded from this leaderboard"}
            ), 409
        ...
        app.logger.info(
            f"Admin {current_user.username} excluded club {club.name} from {leaderboard_type} leaderboard"
        )
        ...
        return jsonify({"success": False, "error": "Failed to add exclusion"}), 500
...
def admin_remove_leaderboard_exclusion(exclusion_id):
    ...
    try:
        ...
        app.logger.info(
            f"Admin {current_user.username} removed exclusion for club {club_name} from {leaderboard_type} leaderboard"
        )

        return jsonify(
            {"success": True, "message": f"Exclusion removed for {club_name}"}
        )

    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Error removing leaderboard exclusion: {str(e)}")
        return jsonify({"success": False, "error": "Failed to remove exclusion"}), 500
...
def admin_create_api_key():
    ...
    api_key = APIKey(
        name=name, description=description, user_id=user.id, rate_limit=rate_limit
    )
    ...
    return jsonify({"message": "API key created successfully", "api_key": api_key.key})
...
def admin_get_oauth_apps():
    ...
    return jsonify(
        {"oauth_apps": oauth_apps_data, "oauth_applications": oauth_apps_data}
    )
...
def admin_create_oauth_app():
    ...
    oauth_app = OAuthApplication(name=name, description=description, user_id=user.id)
        ...
        if action == "approve":
            ...
            try:
                ...
                if not grant_amount_str:
                    return jsonify(
                        {"error": "Grant amount is empty after cleaning"}
                    ), 400
                ...
                return jsonify(
                    {"error": f"Invalid grant amount format: {grant_amount_raw}"}
                ), 400
            ...
            try:
                ...
                if success:
                    app.logger.info(
                        f"Transaction recorded for grant: {int(grant_amount * 100)} tokens credited"
                    )
                else:
                    app.logger.error(
                        f"Failed to record transaction for grant: {tx_result}"
                    )
            except Exception as tx_error:
                app.logger.error(
                    f"Exception while recording grant transaction: {str(tx_error)}"
                )

            app.logger.info(
                f"Added ${grant_amount} to club '{club.name}' (ID: {club.id}) for approved grant {submission_id}"
            )
...
def admin_allocate_tokens_to_existing_clubs():
    ...
    try:
        ...
        clubs_to_allocate = Club.query.filter(Club.airtable_data.isnot(None)).all()
                    ...
                    try:
                        ...
                        if airtable_data and airtable_data.get("airtable_id"):
                            ...
                            airtable_update_data = {
                                "fields": {"Onboarded to Dashboard": True}
                            }
                            ...
                            if response.status_code != 200:
                                app.logger.error(
                                    f"Failed to update Airtable onboarded status for club {club.id}: {response.text}"
                                )

                    except Exception as e:
                        app.logger.error(
                            f"Error updating Airtable onboarded status for club {club.id}: {str(e)}"
                        )
        ...
        app.logger.info(
            f"Admin {current_user.username} allocated tokens to {allocated_count} clubs, {failed_count} failed"
        )
...
def api_get_clubs():
    ...
    if all_clubs:
        ...
        return jsonify({"clubs": clubs_data, "total": len(clubs_data)})
    else:
                    ...
                    return jsonify(
                        {
                            "club": {
                                ...
                                "location": fields.get("Location", ""),
                                "leader": {
                                    "email": fields.get("Current Leaders' Emails", "")
                                    .split(",")[0]
                                    .strip()
                                },
                                "member_count": 0,
    ...
    projects_data = [
        {
            ...
            "github_url": project.github_url,
            "featured": project.featured,
            "author": {"id": project.user.id, "username": project.user.username},
            "created_at": project.created_at.isoformat()
        ...
        clubs_data.append(
            {
                ...
                + ("..." if len(club.description or "") > 100 else ""),
                "leader": {
                    "id": club.leader.id,
                    "username": club.leader.username,
                    "email": club.leader.email,
...
def api_grant_tokens(club_id):
    ...
    try:
        ...
        if amount > 100000:  # 1000 USD limit
            return jsonify(
                {"error": "Amount cannot exceed 100,000 tokens (1000 USD)"}
            ), 400
        ...
        success, result = create_club_transaction(
            ...
            transaction_type="grant",
            amount=amount,
            description=f"Admin token grant: {description}"
            if description
            else "Admin token grant",
        ...
        app.logger.info(
            f"API granted {amount} tokens to club '{club.name}' (ID: {club_id}). New balance: {club.tokens}"
        )
...
def api_remove_tokens(club_id):
    ...
    try:
        ...
        success, result = create_club_transaction(
            ...
            description=f"Admin token removal: {description}"
            if description
            else "Admin token removal",
        ...
        app.logger.warning(
            f"API removed {amount} tokens from club '{club.name}' (ID: {club_id}). New balance: {club.tokens}"
        )
...
def oauth_authorize():
    ...
    if not oauth_app:
        ...
        inactive_app = OAuthApplication.query.filter_by(
            client_id=client_id, is_active=False
        ).first()
        if inactive_app:
            return jsonify(
                {
    ...
    if request.method == "POST":
        ...
        if action == "deny":
            ...
            identity_redirect_uri = url_for(
                "hackclub_identity_callback", _external=True, _scheme="https"
            )
            identity_state = secrets.token_urlsafe(32)
            session["hackclub_identity_state"] = identity_state

            identity_auth_url = hackclub_identity_service.get_auth_url(
                identity_redirect_uri, identity_state
            )
        ...
        scopes_with_descriptions.append(
            {
                ...
                "description": scope_descriptions.get(
                    scope_name, f"Access {scope_name}"
                ),
            }
        )
    ...
    # Determine template to use
    template_name = (
        "oauth_consent_mobile.html"
        if (is_mobile or force_mobile) and not force_desktop
        else "oauth_consent.html"
    )
...
def oauth_token():
    ...
    auth_code = OAuthAuthorizationCode.query.filter_by(
        code=code, application_id=oauth_app.id, redirect_uri=redirect_uri, used=False
    ).first()

    if not auth_code:
        # Check for more specific error cases
        used_code = OAuthAuthorizationCode.query.filter_by(
            code=code, application_id=oauth_app.id, used=True
        ).first()
        ...
        wrong_redirect = OAuthAuthorizationCode.query.filter_by(
            code=code, application_id=oauth_app.id, used=False
        ).first()
    ...
    oauth_token = OAuthToken(user_id=auth_code.user_id, application_id=oauth_app.id)
...
def oauth_user():
    ...
    if user.identity_token:
        ...
        if identity_info and "identity" in identity_info:
            identity_status = identity_info["identity"].get(
                "verification_status", "unverified"
            )
            rejection_reason = identity_info["identity"].get("rejection_reason")
...
def oauth_user_projects():
    ...
    projects = (
        ClubProject.query.filter_by(user_id=user.id)
        .order_by(ClubProject.updated_at.desc())
        .all()
    )
        ...
        projects_data.append(
            {
                ...
                "github_url": project.github_url,
                "featured": project.featured,
                "club": {"id": project.club.id, "name": project.club.name},
                "created_at": project.created_at.isoformat()
                if project.created_at
                else None,
                "updated_at": project.updated_at.isoformat()
    ...
    return jsonify({"projects": projects_data, "total_projects": len(projects_data)})
...
def oauth_user_assignments():
    ...
    member_club_ids = [
        m.club_id for m in ClubMembership.query.filter_by(user_id=user.id).all()
    ]
    all_club_ids = list(set(led_club_ids + member_club_ids))

    # Get assignments from all user's clubs
    assignments = (
        ClubAssignment.query.filter(ClubAssignment.club_id.in_(all_club_ids))
        .order_by(ClubAssignment.created_at.desc())
        .all()
    )
        ...
        assignments_data.append(
            {
                ...
                "status": assignment.status,
                "club": {"id": assignment.club.id, "name": assignment.club.name},
                "created_at": assignment.created_at.isoformat()
    ...
    return jsonify(
        {"assignments": assignments_data, "total_assignments": len(assignments_data)}
    )
...
def oauth_user_meetings():
    ...
    meetings = (
        ClubMeeting.query.filter(ClubMeeting.club_id.in_(all_club_ids))
        .order_by(ClubMeeting.meeting_date.desc())
        .all()
    )
        ...
        meetings_data.append(
            {
                ...
                "location": meeting.location,
                "meeting_link": meeting.meeting_link,
                "club": {"id": meeting.club.id, "name": meeting.club.name},
                "created_at": meeting.created_at.isoformat()
    ...
    return jsonify({"meetings": meetings_data, "total_meetings": len(meetings_data)})
...
def get_settings():
    ...
    try:
        ...
        admin_economy_override = SystemSettings.get_setting(
            "admin_economy_override", "false"
        )
        app.logger.debug("get_settings: Fetching club_creation_enabled setting")
        club_creation_enabled = SystemSettings.get_setting(
            "club_creation_enabled", "true"
        )
        app.logger.debug("get_settings: Fetching user_registration_enabled setting")
        user_registration_enabled = SystemSettings.get_setting(
            "user_registration_enabled", "true"
        )
...
def update_setting():
    ...
    try:
        ...
        valid_keys = [
            "maintenance_mode",
            "economy_enabled",
            "admin_economy_override",
            "club_creation_enabled",
        ...
        if key not in valid_keys:
            return jsonify(
                {"success": False, "message": f"Invalid setting key: {key}"}
            ), 400

        # Validate values
        if value not in ["true", "false"]:
            return jsonify(
                {"success": False, "message": 'Setting value must be "true" or "false"'}
            ), 400
        ...
        app.logger.info(
            f"Setting {key} updated to {value} by admin {current_user.username}"
        )
...
def get_banner_settings():
    ...
    try:
        ...
        settings = {
            ...
            "subtitle": SystemSettings.get_setting(
                "banner_subtitle",
                "Submit your creative projects and win amazing prizes!",
            ),
            "icon": SystemSettings.get_setting("banner_icon", "fas fa-palette"),
            "primary_color": SystemSettings.get_setting(
                "banner_primary_color", "#ec3750"
            ),
            "secondary_color": SystemSettings.get_setting(
                "banner_secondary_color", "#d63146"
            ),
...
def update_banner_settings():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "Banner settings updated successfully"}
        )
...
def get_status_banner():
    ...
    try:
        ...
        active_incidents = (
            ...
            .limit(3)
            .all()
        )  # Only get top 3 most severe incidents
        ...
        return jsonify(
            {"has_active_incidents": False, "incidents": [], "banner_severity": "none"}
        )
...
def update_status_incident(incident_id):
    ...
    try:
        ...
        if "description" in data:
            incident.description = sanitize_string(
                data["description"], allow_html=False
            )
        if "status" in data:
...
def add_status_update(incident_id):
    ...
    try:
        ...
        db.session.rollback()
        app.logger.error(
            f"Error adding status update to incident {incident_id}: {str(e)}"
        )
        return jsonify({"error": "Internal server error"}), 500
